...
public:
	...
	char EInitialOscillatorOffset GetInitialOffset() {
		return memory.read<char EInitialOscillatorOffset>(m_addr + 8);
	}
	uint8_t  GetWaveform() {
...
public:
	...
	struct FMinimalViewInfo GetModifiedPOV() {
		return memory.read<struct FMinimalViewInfo>(m_addr + 1536);
	}
