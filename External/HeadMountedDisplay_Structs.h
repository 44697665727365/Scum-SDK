...
public:
	...
	char EHMDTrackingOrigin GetOrigin() {
		return memory.read<char EHMDTrackingOrigin>(m_addr + 0);
	}
...
public:
	...
	uint8_t  GetDeviceVisualType() {
		return memory.read<uint8_t >(m_addr + 28);
	}
	uint8_t  GetHandIndex() {
		return memory.read<uint8_t >(m_addr + 29);
	}
	uint8_t  GetTrackingStatus() {
...
public:
	...
	char EXRDeviceConnectionResult GetReturnValue() {
		return memory.read<char EXRDeviceConnectionResult>(m_addr + 20);
	}
...
public:
	...
	char EHMDWornState GetReturnValue() {
		return memory.read<char EHMDWornState>(m_addr + 0);
	}
...
public:
	...
	char EHMDTrackingOrigin GetReturnValue() {
		return memory.read<char EHMDTrackingOrigin>(m_addr + 0);
	}
...
public:
	...
	char EOrientPositionSelector GetOptions() {
		return memory.read<char EOrientPositionSelector>(m_addr + 4);
	}
