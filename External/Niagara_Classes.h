...
public:
	...
	uint8_t  GetLockedAxisSpace() {
		return memory.read<uint8_t >(m_addr + 188);
	}
	...
	uint8_t  GetPivotOffsetSpace() {
		return memory.read<uint8_t >(m_addr + 2196);
	}
...
public:
	...
	char ETextureRenderTargetFormat GetOverrideRenderTargetFormat() {
		return memory.read<char ETextureRenderTargetFormat>(m_addr + 228);
	}
...
public:
	...
	uint8_t  GetOverrideBufferFormat() {
		return memory.read<uint8_t >(m_addr + 280);
	}
	char GetbOverrideFormat() {
...
public:
	...
	uint8_t  GetTickBehavior() {
		return memory.read<uint8_t >(m_addr + 1112);
	}
	int32_t GetRandomSeedOffset() {
		return memory.read<int32_t>(m_addr + 1116);
	}
	struct FNiagaraUserRedirectionParameterStore GetOverrideParameters() {
		return memory.read<struct FNiagaraUserRedirectionParameterStore>(m_addr + 1120);
	}
	char GetbForceSolo() {
		return memory.read<char>(m_addr + 1320);
	}
	char GetbEnableGpuComputeDebug() {
		return memory.read<char>(m_addr + 1320);
	}
	char GetbAutoDestroy() {
		return memory.read<char>(m_addr + 1368);
	}
	...
	struct TArray<struct FNiagaraMaterialOverride> GetEmitterMaterials() {
		return memory.read<struct TArray<struct FNiagaraMaterialOverride>>(m_addr + 1376);
	}
	...
	struct TWeakObjectPtr<USceneComponent> GetAutoAttachParent() {
		return memory.read<struct TWeakObjectPtr<USceneComponent>>(m_addr + 1416);
	}
	struct FName GetAutoAttachSocketName() {
		return memory.read<struct FName>(m_addr + 1424);
	}
	uint8_t  GetAutoAttachLocationRule() {
		return memory.read<uint8_t >(m_addr + 1432);
	}
	uint8_t  GetAutoAttachRotationRule() {
		return memory.read<uint8_t >(m_addr + 1433);
	}
	uint8_t  GetAutoAttachScaleRule() {
		return memory.read<uint8_t >(m_addr + 1434);
	}
...
public:
	...
	uint8_t  GetMotionVectorSetting() {
...
public:
	...
	uint8_t  GetSetResolutionMethod() {
		return memory.read<uint8_t >(m_addr + 236);
	}
...
public:
	...
	uint8_t  GetMipMapGeneration() {
		return memory.read<uint8_t >(m_addr + 224);
	}
	char ETextureRenderTargetFormat GetOverrideRenderTargetFormat() {
		return memory.read<char ETextureRenderTargetFormat>(m_addr + 225);
	}
	char GetbInheritUserParameterSettings() {
...
public:
	...
	char ETextureRenderTargetFormat GetOverrideRenderTargetFormat() {
		return memory.read<char ETextureRenderTargetFormat>(m_addr + 220);
	}
	char GetbInheritUserParameterSettings() {
...
public:
	...
	uint8_t  GetDrawDirection() {
		return memory.read<uint8_t >(m_addr + 236);
	}
	uint8_t  GetShape() {
		return memory.read<uint8_t >(m_addr + 237);
	}
	...
	uint8_t  GetTessellationMode() {
		return memory.read<uint8_t >(m_addr + 276);
	}
...
public:
	...
	char ETextureRenderTargetFormat GetDefaultRenderTargetFormat() {
		return memory.read<char ETextureRenderTargetFormat>(m_addr + 176);
	}
	uint8_t  GetDefaultGridFormat() {
		return memory.read<uint8_t >(m_addr + 177);
	}
	uint8_t  GetDefaultRendererMotionVectorSetting() {
		return memory.read<uint8_t >(m_addr + 180);
	}
	char ENDISkelMesh_GpuMaxInfluences GetNDISkelMesh_GpuMaxInfluences() {
		return memory.read<char ENDISkelMesh_GpuMaxInfluences>(m_addr + 184);
	}
	char ENDISkelMesh_GpuUniformSamplingFormat GetNDISkelMesh_GpuUniformSamplingFormat() {
		return memory.read<char ENDISkelMesh_GpuUniformSamplingFormat>(m_addr + 185);
	}
	char ENDISkelMesh_AdjacencyTriangleIndexFormat GetNDISkelMesh_AdjacencyTriangleIndexFormat() {
		return memory.read<char ENDISkelMesh_AdjacencyTriangleIndexFormat>(m_addr + 186);
	}
