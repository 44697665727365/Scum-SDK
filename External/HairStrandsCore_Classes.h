...
public:
	...
	uint8_t  GetHairInterpolationType() {
		return memory.read<uint8_t >(m_addr + 193);
	}
	uint8_t  GetLODSelectionType() {
...
public:
	...
	struct TArray<struct FHairGroupDesc> GetGroomGroupsDesc() {
		return memory.read<struct TArray<struct FHairGroupDesc>>(m_addr + 1344);
	}
	bool GetbRunning() {
