...
public:
	...
	struct FCameraFilmbackSettings GetFilmback() {
		return memory.read<struct FCameraFilmbackSettings>(m_addr + 1444);
	}
	struct FCameraLensSettings GetLensSettings() {
		return memory.read<struct FCameraLensSettings>(m_addr + 1456);
	}
	struct FCameraFocusSettings GetFocusSettings() {
		return memory.read<struct FCameraFocusSettings>(m_addr + 1480);
	}
