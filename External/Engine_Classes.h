...
public:
	...
	uint8_t  GetUpdateOverlapsMethodDuringLevelStreaming() {
		return memory.read<uint8_t >(m_addr + 93);
	}
	uint8_t  GetDefaultUpdateOverlapsMethodDuringLevelStreaming() {
		return memory.read<uint8_t >(m_addr + 94);
	}
	char ENetRole GetRemoteRole() {
		return memory.read<char ENetRole>(m_addr + 95);
	}
	...
	char ENetRole GetRole() {
		return memory.read<char ENetRole>(m_addr + 240);
	}
	char ENetDormancy GetNetDormancy() {
		return memory.read<char ENetDormancy>(m_addr + 241);
	}
	uint8_t  GetSpawnCollisionHandlingMethod() {
		return memory.read<uint8_t >(m_addr + 242);
	}
	char EAutoReceiveInput GetAutoReceiveInput() {
		return memory.read<char EAutoReceiveInput>(m_addr + 243);
	}
...
public:
	...
	char EAutoReceiveInput GetAutoPossessPlayer() {
		return memory.read<char EAutoReceiveInput>(m_addr + 560);
	}
	uint8_t  GetAutoPossessAI() {
		return memory.read<uint8_t >(m_addr + 561);
	}
...
public:
	...
	struct FLightmassPointLightSettings GetLightmassSettings() {
		return memory.read<struct FLightmassPointLightSettings>(m_addr + 820);
	}
...
public:
	...
	struct FSingleAnimationPlayData GetAnimationData() {
		return memory.read<struct FSingleAnimationPlayData>(m_addr + 1728);
	}
	struct FVector GetRootBoneTranslation() {
		return memory.read<struct FVector>(m_addr + 1768);
	}
	struct FVector GetLineCheckBoundsScale() {
		return memory.read<struct FVector>(m_addr + 1780);
	}
	...
	struct TArray<struct FTransform> GetCachedBoneSpaceTransforms() {
		return memory.read<struct TArray<struct FTransform>>(m_addr + 1856);
	}
	struct TArray<struct FTransform> GetCachedComponentSpaceTransforms() {
		return memory.read<struct TArray<struct FTransform>>(m_addr + 1872);
	}
	float GetGlobalAnimRateScale() {
		return memory.read<float>(m_addr + 2224);
	}
	char EKinematicBonesUpdateToPhysics GetKinematicBonesUpdateType() {
		return memory.read<char EKinematicBonesUpdateToPhysics>(m_addr + 2228);
	}
	char EPhysicsTransformUpdateMode GetPhysicsTransformUpdateMode() {
		return memory.read<char EPhysicsTransformUpdateMode>(m_addr + 2229);
	}
	char EAnimationMode GetAnimationMode() {
		return memory.read<char EAnimationMode>(m_addr + 2231);
	}
	char GetbDisablePostProcessBlueprint() {
		return memory.read<char>(m_addr + 2233);
	}
	...
	char GetbDisableClothSimulation() {
		return memory.read<char>(m_addr + 2234);
	}
	char GetbDisableRigidBodyAnimNode() {
		return memory.read<char>(m_addr + 2240);
	}
	...
	char GetbCollideWithAttachedChildren() {
		return memory.read<char>(m_addr + 2241);
	}
	...
	char GetbEnablePerPolyCollision() {
		return memory.read<char>(m_addr + 2242);
	}
	...
	char GetbIncludeComponentLocationIntoBounds() {
		return memory.read<char>(m_addr + 2243);
	}
	...
	uint16_t GetCachedAnimCurveUidVersion() {
		return memory.read<uint16_t>(m_addr + 2246);
	}
	...
	struct TArray<struct FName> GetDisallowedAnimCurves() {
		return memory.read<struct TArray<struct FName>>(m_addr + 2256);
	}
	...
	struct FMulticastInlineDelegate GetOnConstraintBroken() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 2288);
	}
	...
	uint32_t GetLastPoseTickFrame() {
		return memory.read<uint32_t>(m_addr + 2536);
	}
	...
	struct FMulticastInlineDelegate GetOnAnimInitialized() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 2832);
	}
...
public:
	...
	bool GetUseHLODMinDrawDistance() {
	...
	char ESceneDepthPriorityGroup GetDepthPriorityGroup() {
		return memory.read<char ESceneDepthPriorityGroup>(m_addr + 528);
	}
	char ESceneDepthPriorityGroup GetViewOwnerDepthPriorityGroup() {
		return memory.read<char ESceneDepthPriorityGroup>(m_addr + 529);
	}
	char EIndirectLightingCacheQuality GetIndirectLightingCacheQuality() {
		return memory.read<char EIndirectLightingCacheQuality>(m_addr + 530);
	}
	uint8_t  GetLightmapType() {
		return memory.read<uint8_t >(m_addr + 531);
	}
	...
	char GetbGenerateOverlapEvents() {
		return memory.read<char>(m_addr + 533);
	}
	...
	char GetbVisibleInRealTimeSkyCaptures() {
		return memory.read<char>(m_addr + 534);
	}
	...
	char GetbUseAsOccluder() {
		return memory.read<char>(m_addr + 535);
	}
	char GetbSelectable() {
		return memory.read<char>(m_addr + 535);
	}
	...
	char GetbCastShadowAsTwoSided() {
		return memory.read<char>(m_addr + 537);
	}
	...
	char GetbApplyImpulseOnDamage() {
		return memory.read<char>(m_addr + 538);
	}
	...
	char GetbHiddenInSceneCapture() {
		return memory.read<char>(m_addr + 539);
	}
	char EHasCustomNavigableGeometry GetbHasCustomNavigableGeometry() {
		return memory.read<char EHasCustomNavigableGeometry>(m_addr + 540);
	}
	char ECanBeCharacterBase GetCanCharacterStepUpOn() {
		return memory.read<char ECanBeCharacterBase>(m_addr + 542);
	}
	struct FLightingChannels GetLightingChannels() {
		return memory.read<struct FLightingChannels>(m_addr + 543);
	}
	uint8_t  GetCustomDepthStencilWriteMask() {
	...
	struct FCustomPrimitiveData GetCustomPrimitiveData() {
		return memory.read<struct FCustomPrimitiveData>(m_addr + 552);
	}
	struct FCustomPrimitiveData GetCustomPrimitiveDataInternal() {
		return memory.read<struct FCustomPrimitiveData>(m_addr + 568);
	}
	...
	int8_t GetVirtualTextureLodBias() {
		return memory.read<int8_t>(m_addr + 624);
	}
	int8_t GetVirtualTextureCullMips() {
		return memory.read<int8_t>(m_addr + 625);
	}
	int8_t GetVirtualTextureMinCoverage() {
		return memory.read<int8_t>(m_addr + 626);
	}
	uint8_t  GetVirtualTextureRenderPassType() {
		return memory.read<uint8_t >(m_addr + 627);
	}
	...
	struct FBodyInstance GetBodyInstance() {
		return memory.read<struct FBodyInstance>(m_addr + 712);
	}
	struct FMulticastSparseDelegate GetOnComponentHit() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1056);
	}
	struct FMulticastSparseDelegate GetOnComponentBeginOverlap() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1057);
	}
	struct FMulticastSparseDelegate GetOnComponentEndOverlap() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1058);
	}
	struct FMulticastSparseDelegate GetOnComponentWake() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1059);
	}
	struct FMulticastSparseDelegate GetOnComponentSleep() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1060);
	}
	struct FMulticastSparseDelegate GetOnBeginCursorOver() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1062);
	}
	struct FMulticastSparseDelegate GetOnEndCursorOver() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1063);
	}
	struct FMulticastSparseDelegate GetOnClicked() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1064);
	}
	struct FMulticastSparseDelegate GetOnReleased() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1065);
	}
	struct FMulticastSparseDelegate GetOnInputTouchBegin() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1066);
	}
	struct FMulticastSparseDelegate GetOnInputTouchEnd() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1067);
	}
	struct FMulticastSparseDelegate GetOnInputTouchEnter() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1068);
	}
	struct FMulticastSparseDelegate GetOnInputTouchLeave() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 1069);
	}
...
public:
	...
	struct FLightingChannels GetLightingChannels() {
		return memory.read<struct FLightingChannels>(m_addr + 608);
	}
	...
	char GetbEnableLightShaftBloom() {
		return memory.read<char>(m_addr + 664);
	}
	float GetBloomScale() {
	...
	struct FColor GetBloomTint() {
		return memory.read<struct FColor>(m_addr + 680);
	}
...
public:
	...
	char ERootMotionMode GetRootMotionMode() {
		return memory.read<char ERootMotionMode>(m_addr + 48);
	}
...
public:
	...
	char EComponentMobility GetMobility() {
		return memory.read<char EComponentMobility>(m_addr + 335);
	}
	char EDetailMode GetDetailMode() {
		return memory.read<char EDetailMode>(m_addr + 336);
	}
	struct FMulticastSparseDelegate GetPhysicsVolumeChangedDelegate() {
		return memory.read<struct FMulticastSparseDelegate>(m_addr + 337);
	}
...
public:
	...
	char EBeam2Method GetBeamMethod() {
		return memory.read<char EBeam2Method>(m_addr + 48);
	}
	...
	char EBeamTaperMethod GetTaperMethod() {
		return memory.read<char EBeamTaperMethod>(m_addr + 144);
	}
...
public:
	...
	struct TWeakObjectPtr<USkinnedMeshComponent> GetMasterPoseComponent() {
		return memory.read<struct TWeakObjectPtr<USkinnedMeshComponent>>(m_addr + 1160);
	}
	struct TArray<uint8_t > GetSkinCacheUsage() {
		return memory.read<struct TArray<uint8_t >>(m_addr + 1168);
	}
	struct TArray<struct FVertexOffsetUsage> GetVertexOffsetUsage() {
		return memory.read<struct TArray<struct FVertexOffsetUsage>>(m_addr + 1184);
	}
	...
	int32_t GetMinLodModel() {
		return memory.read<int32_t>(m_addr + 1460);
	}
	...
	struct TArray<struct FSkelMeshComponentLODInfo> GetLODInfo() {
		return memory.read<struct TArray<struct FSkelMeshComponentLODInfo>>(m_addr + 1488);
	}
	uint8_t  GetVisibilityBasedAnimTickOption() {
		return memory.read<uint8_t >(m_addr + 1540);
	}
	char GetbOverrideMinLOD() {
		return memory.read<char>(m_addr + 1542);
	}
	...
	char GetbHideSkin() {
		return memory.read<char>(m_addr + 1543);
	}
	...
	char GetbCastCapsuleIndirectShadow() {
		return memory.read<char>(m_addr + 1544);
	}
	...
	char GetbCachedLocalBoundsUpToDate() {
		return memory.read<char>(m_addr + 1545);
	}
	...
	struct FBoxSphereBounds GetCachedWorldSpaceBounds() {
		return memory.read<struct FBoxSphereBounds>(m_addr + 1568);
	}
	struct FMatrix GetCachedWorldToLocalTransform() {
		return memory.read<struct FMatrix>(m_addr + 1600);
	}
...
public:
	...
	uint8_t  GetPlaneConstraintAxisSetting() {
		return memory.read<uint8_t >(m_addr + 235);
	}
...
public:
	...
	char GetbEnableMaterialParameterCaching() {
		return memory.read<char>(m_addr + 1136);
	}
...
public:
	...
	struct FGuid GetLightGuid() {
		return memory.read<struct FGuid>(m_addr + 504);
	}
	float GetBrightness() {
		return memory.read<float>(m_addr + 520);
	}
...
public:
	...
	struct FSplineMeshParams GetSplineParams() {
		return memory.read<struct FSplineMeshParams>(m_addr + 1256);
	}
	...
	char GetbAllowSplineEditingPerInstance() {
		return memory.read<char>(m_addr + 1388);
	}
	...
	char ESplineMeshAxis GetForwardAxis() {
		return memory.read<char ESplineMeshAxis>(m_addr + 1389);
	}
...
public:
	...
	struct TArray<struct FAudioComponentParam> GetInstanceParameters() {
		return memory.read<struct TArray<struct FAudioComponentParam>>(m_addr + 512);
	}
	...
	struct FSoundAttenuationSettings GetAttenuationOverrides() {
		return memory.read<struct FSoundAttenuationSettings>(m_addr + 624);
	}
	...
	uint8_t  GetAutoAttachLocationRule() {
		return memory.read<uint8_t >(m_addr + 1652);
	}
	uint8_t  GetAutoAttachRotationRule() {
		return memory.read<uint8_t >(m_addr + 1653);
	}
	uint8_t  GetAutoAttachScaleRule() {
		return memory.read<uint8_t >(m_addr + 1654);
	}
	struct FSoundModulationDefaultRoutingSettings GetModulationRouting() {
		return memory.read<struct FSoundModulationDefaultRoutingSettings>(m_addr + 1656);
	}
	...
	struct FMulticastInlineDelegate GetOnAudioFinished() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1808);
	}
	struct FMulticastInlineDelegate GetOnAudioPlaybackPercent() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1848);
	}
	struct FMulticastInlineDelegate GetOnAudioSingleEnvelopeValue() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1888);
	}
	struct FMulticastInlineDelegate GetOnAudioMultiEnvelopeValue() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1928);
	}
	struct FDelegate GetOnQueueSubtitles() {
		return memory.read<struct FDelegate>(m_addr + 1968);
	}
	struct TWeakObjectPtr<USceneComponent> GetAutoAttachParent() {
		return memory.read<struct TWeakObjectPtr<USceneComponent>>(m_addr + 1984);
	}
	struct FName GetAutoAttachSocketName() {
		return memory.read<struct FName>(m_addr + 1992);
	}
...
public:
	...
	int32_t GetbClampBandwithDeltaTime() {
		return memory.read<int32_t>(m_addr + 88);
	}
	float GetMinBandwithDeltaTime() {
		return memory.read<float>(m_addr + 92);
	}
	float GetDeltaBitsMultiplier() {
		return memory.read<float>(m_addr + 96);
	}
	...
	struct FName GetNetDriverName() {
		return memory.read<struct FName>(m_addr + 416);
	}
	struct TArray<struct FChannelDefinition> GetChannelDefinitions() {
		return memory.read<struct TArray<struct FChannelDefinition>>(m_addr + 424);
	}
	struct TMap<struct FName, struct FChannelDefinition> GetChannelDefinitionMap() {
		return memory.read<struct TMap<struct FName, struct FChannelDefinition>>(m_addr + 440);
	}
...
public:
	...
	uint8_t  GetDefaultViewportMouseCaptureMode() {
		return memory.read<uint8_t >(m_addr + 112);
	}
	uint8_t  GetDefaultViewportMouseLockMode() {
...
public:
	...
	uint8_t  GetSampleRateQuality() {
		return memory.read<uint8_t >(m_addr + 376);
	}
	char ESoundGroup GetSoundGroup() {
		return memory.read<char ESoundGroup>(m_addr + 378);
	}
	char GetbLooping() {
	...
	uint8_t  GetLoadingBehavior() {
		return memory.read<uint8_t >(m_addr + 380);
	}
	char GetbMature() {
...
public:
	...
	struct FExponentialHeightFogData GetSecondFogData() {
		return memory.read<struct FExponentialHeightFogData>(m_addr + 512);
	}
	struct FLinearColor GetFogInscatteringColor() {
		return memory.read<struct FLinearColor>(m_addr + 524);
	}
	...
	struct FLinearColor GetInscatteringTextureTint() {
		return memory.read<struct FLinearColor>(m_addr + 556);
	}
	...
	struct FLinearColor GetDirectionalInscatteringColor() {
		return memory.read<struct FLinearColor>(m_addr + 588);
	}
	...
	struct FColor GetVolumetricFogAlbedo() {
		return memory.read<struct FColor>(m_addr + 624);
	}
	...
	bool GetbOverrideLightColorsWithFogInscatteringColors() {
		return memory.read<bool>(m_addr + 656);
	}
...
public:
	...
	char EInterpMoveAxis GetMoveAxis() {
		return memory.read<char EInterpMoveAxis>(m_addr + 144);
	}
...
public:
	...
	char EPixelFormat GetOverrideFormat() {
		return memory.read<char EPixelFormat>(m_addr + 404);
	}
	char GetbHDR() {
...
public:
	...
	char EBrushType GetBrushType() {
		return memory.read<char EBrushType>(m_addr + 544);
	}
...
public:
	...
	uint32_t GetReplicatedCrouchedStateAndCapsuleHalfHeight() {
		return memory.read<uint32_t>(m_addr + 820);
	}
	char GetbProxyIsJumpForceApplied() {
		return memory.read<char>(m_addr + 824);
	}
	...
	char GetbServerMoveIgnoreRootMotion() {
		return memory.read<char>(m_addr + 825);
	}
	char GetbWasJumping() {
		return memory.read<char>(m_addr + 825);
	}
	...
	int32_t GetJumpCurrentCountPreJump() {
		return memory.read<int32_t>(m_addr + 852);
	}
	struct FMulticastInlineDelegate GetOnReachedJumpApex() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 864);
	}
	struct FMulticastInlineDelegate GetMovementModeChangedDelegate() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 896);
	}
	struct FMulticastInlineDelegate GetOnCharacterMovementUpdated() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 912);
	}
	struct FRootMotionSourceGroup GetSavedRootMotion() {
		return memory.read<struct FRootMotionSourceGroup>(m_addr + 928);
	}
	struct FRootMotionMovementParams GetClientRootMotionParams() {
		return memory.read<struct FRootMotionMovementParams>(m_addr + 992);
	}
	struct TArray<struct FSimulatedRootMotionReplicatedMove> GetRootMotionRepMoves() {
		return memory.read<struct TArray<struct FSimulatedRootMotionReplicatedMove>>(m_addr + 1056);
	}
	struct FRepRootMotionMontage GetRepRootMotion() {
		return memory.read<struct FRepRootMotionMontage>(m_addr + 1072);
	}
...
public:
	...
	char EVisibilityAggressiveness GetVisibilityAggressiveness() {
		return memory.read<char EVisibilityAggressiveness>(m_addr + 556);
	}
	...
	struct TArray<struct FNetViewer> GetReplicationViewers() {
		return memory.read<struct TArray<struct FNetViewer>>(m_addr + 816);
	}
	struct TArray<struct UAssetUserData> GetAssetUserData() {
...
public:
	...
	struct FWorldPSCPool GetPSCPool() {
		return memory.read<struct FWorldPSCPool>(m_addr + 1680);
	}
...
public:
	...
	char ETimelineLengthMode GetLengthMode() {
		return memory.read<char ETimelineLengthMode>(m_addr + 44);
	}
	char GetbAutoPlay() {
	...
	char ETickingGroup GetTimelineTickGroup() {
		return memory.read<char ETickingGroup>(m_addr + 144);
	}
...
public:
	...
	struct FSplineCurves GetSplineCurves() {
		return memory.read<struct FSplineCurves>(m_addr + 1104);
	}
	struct FInterpCurveVector GetSplineInfo() {
		return memory.read<struct FInterpCurveVector>(m_addr + 1216);
	}
	struct FInterpCurveQuat GetSplineRotInfo() {
		return memory.read<struct FInterpCurveQuat>(m_addr + 1240);
	}
	struct FInterpCurveVector GetSplineScaleInfo() {
		return memory.read<struct FInterpCurveVector>(m_addr + 1264);
	}
	struct FInterpCurveFloat GetSplineReparamTable() {
		return memory.read<struct FInterpCurveFloat>(m_addr + 1288);
	}
	bool GetbAllowSplineEditingPerInstance() {
		return memory.read<bool>(m_addr + 1312);
	}
	int32_t GetReparamStepsPerSegment() {
		return memory.read<int32_t>(m_addr + 1316);
	}
	float GetDuration() {
	...
	bool GetbInputSplinePointsToConstructionScript() {
		return memory.read<bool>(m_addr + 1327);
	}
	bool GetbDrawDebug() {
		return memory.read<bool>(m_addr + 1328);
	}
	bool GetbClosedLoop() {
		return memory.read<bool>(m_addr + 1329);
	}
	bool GetbLoopPositionOverride() {
		return memory.read<bool>(m_addr + 1330);
	}
...
public:
	...
	char EMovementMode GetMovementMode() {
		return memory.read<char EMovementMode>(m_addr + 360);
	}
	char GetCustomMovementMode() {
		return memory.read<char>(m_addr + 361);
	}
	uint8_t  GetNetworkSmoothingMode() {
		return memory.read<uint8_t >(m_addr + 362);
	}
	...
	char EMovementMode GetDefaultLandMovementMode() {
		return memory.read<char EMovementMode>(m_addr + 900);
	}
	char EMovementMode GetDefaultWaterMovementMode() {
		return memory.read<char EMovementMode>(m_addr + 901);
	}
	char EMovementMode GetGroundMovementMode() {
		return memory.read<char EMovementMode>(m_addr + 902);
	}
...
public:
	...
	struct FEquirectProps GetEquirectProps() {
		return memory.read<struct FEquirectProps>(m_addr + 572);
	}
	char EStereoLayerType GetStereoLayerType() {
		return memory.read<char EStereoLayerType>(m_addr + 644);
	}
	char EStereoLayerShape GetStereoLayerShape() {
		return memory.read<char EStereoLayerShape>(m_addr + 645);
	}
...
public:
	...
	struct TArray<struct FInstancedStaticMeshInstanceData> GetPerInstanceSMData() {
		return memory.read<struct TArray<struct FInstancedStaticMeshInstanceData>>(m_addr + 1248);
	}
	int32_t GetNumCustomDataFloats() {
		return memory.read<int32_t>(m_addr + 1264);
	}
	bool GetbKeepInstanceBufferCPUAccess() {
		return memory.read<bool>(m_addr + 1268);
	}
	struct TArray<float> GetPerInstanceSMCustomData() {
		return memory.read<struct TArray<float>>(m_addr + 1272);
	}
	int32_t GetInstancingRandomSeed() {
		return memory.read<int32_t>(m_addr + 1288);
	}
	int32_t GetInstanceStartCullDistance() {
		return memory.read<int32_t>(m_addr + 1292);
	}
	int32_t GetInstanceEndCullDistance() {
		return memory.read<int32_t>(m_addr + 1296);
	}
	struct TArray<int32_t> GetInstanceReorderTable() {
		return memory.read<struct TArray<int32_t>>(m_addr + 1304);
	}
	int32_t GetNumPendingLightmaps() {
		return memory.read<int32_t>(m_addr + 1392);
	}
	struct TArray<struct FInstancedStaticMeshMappingInfo> GetCachedMappings() {
		return memory.read<struct TArray<struct FInstancedStaticMeshMappingInfo>>(m_addr + 1400);
	}
...
public:
	...
	char EParticleCollisionResponse GetResponse() {
		return memory.read<char EParticleCollisionResponse>(m_addr + 164);
	}
	char EParticleCollisionMode GetCollisionMode() {
		return memory.read<char EParticleCollisionMode>(m_addr + 165);
	}
...
public:
	...
	char EAspectRatioAxisConstraint GetAspectRatioAxisConstraint() {
		return memory.read<char EAspectRatioAxisConstraint>(m_addr + 148);
	}
...
public:
	...
	char GetbPlayerIsWaiting() {
		return memory.read<char>(m_addr + 976);
	}
	char GetNetPlayerIndex() {
		return memory.read<char>(m_addr + 980);
	}
	...
	char GetbShowMouseCursor() {
		return memory.read<char>(m_addr + 1080);
	}
	...
	struct TArray<struct FKey> GetClickEventKeys() {
		return memory.read<struct TArray<struct FKey>>(m_addr + 1088);
	}
	char EMouseCursor GetDefaultMouseCursor() {
		return memory.read<char EMouseCursor>(m_addr + 1104);
	}
	char EMouseCursor GetCurrentMouseCursor() {
		return memory.read<char EMouseCursor>(m_addr + 1105);
	}
	char ECollisionChannel GetDefaultClickTraceChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 1106);
	}
	char ECollisionChannel GetCurrentClickTraceChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 1107);
	}
	float GetHitResultTraceDistance() {
		return memory.read<float>(m_addr + 1108);
	}
	uint16_t GetSeamlessTravelCount() {
		return memory.read<uint16_t>(m_addr + 1112);
	}
	uint16_t GetLastCompletedSeamlessTravelCount() {
		return memory.read<uint16_t>(m_addr + 1114);
	}
	...
	char GetbShouldPerformFullTickWhenPaused() {
		return memory.read<char>(m_addr + 1240);
	}
	...
	bool GetbIsLocalPlayerController() {
		return memory.read<bool>(m_addr + 1364);
	}
...
public:
	...
	struct FColor GetWireframeColorOverride() {
		return memory.read<struct FColor>(m_addr + 1168);
	}
	char GetbEvaluateWorldPositionOffset() {
		return memory.read<char>(m_addr + 1172);
	}
	...
	char GetbCastDistanceFieldIndirectShadow() {
		return memory.read<char>(m_addr + 1173);
	}
	...
	int32_t GetOverriddenLightMapRes() {
		return memory.read<int32_t>(m_addr + 1176);
	}
	...
	struct TArray<struct FStaticMeshComponentLODInfo> GetLODData() {
		return memory.read<struct TArray<struct FStaticMeshComponentLODInfo>>(m_addr + 1192);
	}
	struct TArray<struct FStreamingTextureBuildInfo> GetStreamingTextureData() {
		return memory.read<struct TArray<struct FStreamingTextureBuildInfo>>(m_addr + 1208);
	}
	struct FLightmassPrimitiveSettings GetLightmassSettings() {
		return memory.read<struct FLightmassPrimitiveSettings>(m_addr + 1224);
	}
...
public:
	...
	struct FCameraCacheEntry GetLastFrameCameraCache() {
		return memory.read<struct FCameraCacheEntry>(m_addr + 2192);
	}
	struct FTViewTarget GetViewTarget() {
		return memory.read<struct FTViewTarget>(m_addr + 3728);
	}
	struct FTViewTarget GetPendingViewTarget() {
		return memory.read<struct FTViewTarget>(m_addr + 5280);
	}
	struct FCameraCacheEntry GetCameraCachePrivate() {
		return memory.read<struct FCameraCacheEntry>(m_addr + 6880);
	}
	struct FCameraCacheEntry GetLastFrameCameraCachePrivate() {
		return memory.read<struct FCameraCacheEntry>(m_addr + 8416);
	}
	struct TArray<struct UCameraModifier> GetModifierList() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 9952);
		return struct TArray<struct UCameraModifier>(ptr_addr);
	}
	struct TArray<UCameraModifier> GetDefaultModifiers() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 9968);
		return struct TArray<UCameraModifier>(ptr_addr);
	}
	float GetFreeCamDistance() {
		return memory.read<float>(m_addr + 9984);
	}
	struct FVector GetFreeCamOffset() {
		return memory.read<struct FVector>(m_addr + 9988);
	}
	struct FVector GetViewTargetOffset() {
		return memory.read<struct FVector>(m_addr + 10000);
	}
	struct FMulticastInlineDelegate GetOnAudioFadeChangeEvent() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 10016);
	}
	struct TArray<struct AEmitterCameraLensEffectBase> GetCameraLensEffects() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 10048);
	...
	struct UCameraModifier_CameraShake GetCachedCameraShakeMod() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 10064);
	...
	struct TArray<struct FPostProcessSettings> GetPostProcessBlendCache() {
		return memory.read<struct TArray<struct FPostProcessSettings>>(m_addr + 10136);
	}
	struct TArray<struct UCameraAnimInst> GetActiveAnims() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 10168);
		return struct TArray<struct UCameraAnimInst>(ptr_addr);
	}
	struct TArray<struct UCameraAnimInst> GetFreeAnims() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 10184);
		return struct TArray<struct UCameraAnimInst>(ptr_addr);
	}
	struct ACameraActor GetAnimCameraActor() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 10200);
		return struct ACameraActor(ptr_addr);
	}
	char GetbIsOrthographic() {
		return memory.read<char>(m_addr + 10208);
	}
	...
	char GetbGameCameraCutThisFrame() {
		return memory.read<char>(m_addr + 10209);
	}
	float GetViewPitchMin() {
		return memory.read<float>(m_addr + 10212);
	}
	float GetViewPitchMax() {
		return memory.read<float>(m_addr + 10216);
	}
	float GetViewYawMin() {
		return memory.read<float>(m_addr + 10220);
	}
	float GetViewYawMax() {
		return memory.read<float>(m_addr + 10224);
	}
	float GetViewRollMin() {
		return memory.read<float>(m_addr + 10228);
	}
	float GetViewRollMax() {
		return memory.read<float>(m_addr + 10232);
	}
	float GetServerUpdateCameraTimeout() {
		return memory.read<float>(m_addr + 10240);
	}
...
public:
	...
	struct FText GetText() {
		return memory.read<struct FText>(m_addr + 1104);
	}
	...
	char EHorizTextAligment GetHorizontalAlignment() {
		return memory.read<char EHorizTextAligment>(m_addr + 1144);
	}
	char EVerticalTextAligment GetVerticalAlignment() {
		return memory.read<char EVerticalTextAligment>(m_addr + 1145);
	}
	struct FColor GetTextRenderColor() {
		return memory.read<struct FColor>(m_addr + 1148);
	}
	float GetXScale() {
		return memory.read<float>(m_addr + 1152);
	}
...
public:
	...
	char EOrbitChainMode GetChainMode() {
		return memory.read<char EOrbitChainMode>(m_addr + 56);
	}
...
public:
	...
	struct TArray<struct FMaterialSpriteElement> GetElements() {
		return memory.read<struct TArray<struct FMaterialSpriteElement>>(m_addr + 1104);
	}
...
public:
	...
	char ECollisionChannel GetProbeChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 540);
	}
...
public:
	...
	char EAutoReceiveInput GetAutoActivateForPlayer() {
		return memory.read<char EAutoReceiveInput>(m_addr + 544);
	}
...
public:
	...
	struct FColor GetShapeColor() {
		return memory.read<struct FColor>(m_addr + 1120);
	}
	char GetbDrawOnlyIfSelected() {
		return memory.read<char>(m_addr + 1124);
	}
...
public:
	...
	char EClampMode GetClampMode() {
		return memory.read<char EClampMode>(m_addr + 124);
	}
	float GetMinDefault() {
		return memory.read<float>(m_addr + 128);
	}
...
public:
	...
	struct TArray<int32_t> GetSortedInstances() {
		return memory.read<struct TArray<int32_t>>(m_addr + 1432);
	}
	int32_t GetNumBuiltInstances() {
		return memory.read<int32_t>(m_addr + 1448);
	}
	struct FBox GetBuiltInstanceBounds() {
		return memory.read<struct FBox>(m_addr + 1456);
	}
	struct FBox GetUnbuiltInstanceBounds() {
		return memory.read<struct FBox>(m_addr + 1484);
	}
	struct TArray<struct FBox> GetUnbuiltInstanceBoundsList() {
		return memory.read<struct TArray<struct FBox>>(m_addr + 1512);
	}
	char GetbEnableDensityScaling() {
		return memory.read<char>(m_addr + 1528);
	}
	int32_t GetOcclusionLayerNumNodes() {
		return memory.read<int32_t>(m_addr + 1536);
	}
	struct FBoxSphereBounds GetCacheMeshExtendedBounds() {
		return memory.read<struct FBoxSphereBounds>(m_addr + 1540);
	}
...
public:
	...
	char BeamModifierType GetModifierType() {
		return memory.read<char BeamModifierType>(m_addr + 48);
	}
...
public:
	...
	char ECameraProjectionMode GetProjectionMode() {
		return memory.read<char ECameraProjectionMode>(m_addr + 525);
	}
	float GetPostProcessBlendWeight() {
		return memory.read<float>(m_addr + 576);
	}
	struct FPostProcessSettings GetPostProcessSettings() {
		return memory.read<struct FPostProcessSettings>(m_addr + 624);
	}
...
public:
	...
	char EEdGraphPinDirection GetDirection() {
		return memory.read<char EEdGraphPinDirection>(m_addr + 72);
	}
...
public:
	...
	struct FVector GetBoxExtent() {
		return memory.read<struct FVector>(m_addr + 1128);
	}
...
public:
	...
	char GetbEnabled() {
		return memory.read<char>(m_addr + 1996);
	}
	char GetbUnbound() {
		return memory.read<char>(m_addr + 1996);
	}
...
public:
	...
	char EParticleAxisLock GetLockAxisFlags() {
		return memory.read<char EParticleAxisLock>(m_addr + 48);
	}
...
public:
	...
	char TextureCompressionSettings GetCompressionSettings() {
		return memory.read<char TextureCompressionSettings>(m_addr + 124);
	}
	char TextureFilter GetFilter() {
		return memory.read<char TextureFilter>(m_addr + 125);
	}
	uint8_t  GetMipLoadOptions() {
		return memory.read<uint8_t >(m_addr + 126);
	}
	char TextureGroup GetLODGroup() {
		return memory.read<char TextureGroup>(m_addr + 127);
	}
	struct FPerPlatformFloat GetDownscale() {
		return memory.read<struct FPerPlatformFloat>(m_addr + 128);
	}
	uint8_t  GetDownscaleOptions() {
		return memory.read<uint8_t >(m_addr + 132);
	}
	char GetsRGB() {
		return memory.read<char>(m_addr + 133);
	}
	char GetbNoTiling() {
		return memory.read<char>(m_addr + 133);
	}
...
public:
	...
	char EUserDefinedStructureStatus GetStatus() {
		return memory.read<char EUserDefinedStructureStatus>(m_addr + 192);
	}
...
public:
	...
	uint8_t  GetNavigationGeometryGatheringMode() {
		return memory.read<uint8_t >(m_addr + 553);
	}
...
public:
	...
	char CylinderHeightAxis GetHeightAxis() {
		return memory.read<char CylinderHeightAxis>(m_addr + 280);
	}
...
public:
	...
	struct TArray<struct FMaterialTextureInfo> GetTextureStreamingData() {
		return memory.read<struct TArray<struct FMaterialTextureInfo>>(m_addr + 96);
	}
	struct TArray<struct UAssetUserData> GetAssetUserData() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 112);
		return struct TArray<struct UAssetUserData>(ptr_addr);
...
public:
	...
	struct TArray<struct FScalarParameterValue> GetScalarParameterValues() {
		return memory.read<struct TArray<struct FScalarParameterValue>>(m_addr + 224);
	}
	struct TArray<struct FVectorParameterValue> GetVectorParameterValues() {
		return memory.read<struct TArray<struct FVectorParameterValue>>(m_addr + 240);
	}
	struct TArray<struct FTextureParameterValue> GetTextureParameterValues() {
		return memory.read<struct TArray<struct FTextureParameterValue>>(m_addr + 256);
	}
	struct TArray<struct FRuntimeVirtualTextureParameterValue> GetRuntimeVirtualTextureParameterValues() {
		return memory.read<struct TArray<struct FRuntimeVirtualTextureParameterValue>>(m_addr + 272);
	}
	struct TArray<struct FFontParameterValue> GetFontParameterValues() {
		return memory.read<struct TArray<struct FFontParameterValue>>(m_addr + 288);
	}
	struct FMaterialInstanceBasePropertyOverrides GetBasePropertyOverrides() {
		return memory.read<struct FMaterialInstanceBasePropertyOverrides>(m_addr + 304);
	}
	struct FStaticParameterSet GetStaticParameters() {
		return memory.read<struct FStaticParameterSet>(m_addr + 328);
	}
	struct FMaterialCachedParameters GetCachedLayerParameters() {
		return memory.read<struct FMaterialCachedParameters>(m_addr + 392);
	}
...
public:
	...
	char EBlueprintType GetBlueprintType() {
		return memory.read<char EBlueprintType>(m_addr + 88);
	}
...
public:
	...
	char EDistributionVectorLockFlags GetLockedAxes() {
		return memory.read<char EDistributionVectorLockFlags>(m_addr + 84);
	}
	char EDistributionVectorMirrorFlags GetMirrorFlags[3]() {
		return memory.read<char EDistributionVectorMirrorFlags>(m_addr + 85);
	}
...
public:
	...
	char ENotifyTriggerMode GetNotifyTriggerMode() {
		return memory.read<char ENotifyTriggerMode>(m_addr + 172);
	}
...
public:
	...
	char EBlendSpaceAxis GetAxisToScaleAnimation() {
		return memory.read<char EBlendSpaceAxis>(m_addr + 328);
	}
...
public:
	...
	char EMaterialExposedViewProperty GetProperty() {
		return memory.read<char EMaterialExposedViewProperty>(m_addr + 64);
	}
...
public:
	...
	struct TMap<struct FName, uint8_t > GetAttributeBlendModes() {
		return memory.read<struct TMap<struct FName, uint8_t >>(m_addr + 120);
	}
	uint8_t  GetDefaultAttributeBlendMode() {
...
public:
	...
	char ECustomMaterialOutputType GetOutputType() {
		return memory.read<char ECustomMaterialOutputType>(m_addr + 80);
	}
...
public:
	...
	char EGrammaticalGender GetGender() {
		return memory.read<char EGrammaticalGender>(m_addr + 40);
	}
	char EGrammaticalNumber GetPlurality() {
		return memory.read<char EGrammaticalNumber>(m_addr + 41);
	}
	struct FGuid GetLocalizationGUID() {
...
public:
	...
	char EParticleSystemUpdateMode GetSystemUpdateMode() {
		return memory.read<char EParticleSystemUpdateMode>(m_addr + 178);
	}
	char ParticleSystemLODMethod GetLODMethod() {
		return memory.read<char ParticleSystemLODMethod>(m_addr + 179);
	}
	uint8_t  GetInsignificantReaction() {
		return memory.read<uint8_t >(m_addr + 180);
	}
	char EParticleSystemOcclusionBoundsMethod GetOcclusionBoundsMethod() {
		return memory.read<char EParticleSystemOcclusionBoundsMethod>(m_addr + 181);
	}
	uint8_t  GetMaxSignificanceLevel() {
		return memory.read<uint8_t >(m_addr + 183);
	}
...
public:
	...
	char TextureAddress GetAddressX() {
		return memory.read<char TextureAddress>(m_addr + 385);
	}
	char TextureAddress GetAddressY() {
		return memory.read<char TextureAddress>(m_addr + 386);
	}
...
public:
	...
	char GetbEnableLightShaftOcclusion() {
		return memory.read<char>(m_addr + 812);
	}
	...
	struct FLinearColor GetAtmosphereSunDiskColorScale() {
		return memory.read<struct FLinearColor>(m_addr + 904);
	}
	...
	struct FLightmassDirectionalLightSettings GetLightmassSettings() {
		return memory.read<struct FLightmassDirectionalLightSettings>(m_addr + 968);
	}
	char GetbCastModulatedShadows() {
		return memory.read<char>(m_addr + 984);
	}
	struct FColor GetModulatedShadowColor() {
		return memory.read<struct FColor>(m_addr + 988);
	}
...
public:
	...
	struct FColor GetFrustumColor() {
		return memory.read<struct FColor>(m_addr + 1104);
	}
...
public:
	...
	char AnimationCompressionFormat GetTranslationCompressionFormat() {
		return memory.read<char AnimationCompressionFormat>(m_addr + 60);
	}
	char AnimationCompressionFormat GetRotationCompressionFormat() {
		return memory.read<char AnimationCompressionFormat>(m_addr + 61);
	}
	char AnimationCompressionFormat GetScaleCompressionFormat() {
		return memory.read<char AnimationCompressionFormat>(m_addr + 62);
	}
...
public:
	...
	struct TArray<char AnimationCompressionFormat> GetAllowedRotationFormats() {
		return memory.read<struct TArray<char AnimationCompressionFormat>>(m_addr + 112);
	}
	struct TArray<char AnimationCompressionFormat> GetAllowedTranslationFormats() {
		return memory.read<struct TArray<char AnimationCompressionFormat>>(m_addr + 128);
	}
	struct TArray<char AnimationCompressionFormat> GetAllowedScaleFormats() {
		return memory.read<struct TArray<char AnimationCompressionFormat>>(m_addr + 144);
	}
...
public:
	...
	char ETrailsRenderAxisOption GetRenderAxis() {
		return memory.read<char ETrailsRenderAxisOption>(m_addr + 68);
	}
...
public:
	...
	char ERootMotionRootLock GetRootMotionRootLock() {
		return memory.read<char ERootMotionRootLock>(m_addr + 379);
	}
...
public:
	...
	char GetbIsScreenSizeScaled() {
		return memory.read<char>(m_addr + 1112);
	}
	float GetScreenSize() {
		return memory.read<float>(m_addr + 1116);
	}
...
public:
	...
	uint8_t  GetMaterialFunctionUsage() {
...
public:
	...
	char ETrailWidthMode GetWidthScaleMode() {
		return memory.read<char ETrailWidthMode>(m_addr + 72);
	}
...
public:
	...
	struct FScalarMaterialInput GetMetallic() {
		return memory.read<struct FScalarMaterialInput>(m_addr + 216);
	}
	struct FScalarMaterialInput GetSpecular() {
		return memory.read<struct FScalarMaterialInput>(m_addr + 236);
	}
	struct FScalarMaterialInput GetAnisotropy() {
		return memory.read<struct FScalarMaterialInput>(m_addr + 256);
	}
	struct FVectorMaterialInput GetNormal() {
		return memory.read<struct FVectorMaterialInput>(m_addr + 276);
	}
	struct FVectorMaterialInput GetTangent() {
		return memory.read<struct FVectorMaterialInput>(m_addr + 296);
	}
	struct FColorMaterialInput GetEmissiveColor() {
		return memory.read<struct FColorMaterialInput>(m_addr + 316);
	}
	char EMaterialDomain GetMaterialDomain() {
		return memory.read<char EMaterialDomain>(m_addr + 336);
	}
	char EBlendMode GetBlendMode() {
		return memory.read<char EBlendMode>(m_addr + 337);
	}
	char EDecalBlendMode GetDecalBlendMode() {
		return memory.read<char EDecalBlendMode>(m_addr + 338);
	}
	char EMaterialDecalResponse GetMaterialDecalResponse() {
		return memory.read<char EMaterialDecalResponse>(m_addr + 339);
	}
	char EMaterialShadingModel GetShadingModel() {
		return memory.read<char EMaterialShadingModel>(m_addr + 340);
	}
	char GetbCastDynamicShadowAsMasked() {
		return memory.read<char>(m_addr + 341);
	}
	struct FMaterialShadingModelField GetShadingModels() {
		return memory.read<struct FMaterialShadingModelField>(m_addr + 342);
	}
	float GetOpacityMaskClipValue() {
		return memory.read<float>(m_addr + 344);
	}
	struct FVectorMaterialInput GetWorldPositionOffset() {
		return memory.read<struct FVectorMaterialInput>(m_addr + 348);
	}
	struct FScalarMaterialInput GetRefraction() {
		return memory.read<struct FScalarMaterialInput>(m_addr + 368);
	}
	struct FMaterialAttributesInput GetMaterialAttributes() {
		return memory.read<struct FMaterialAttributesInput>(m_addr + 388);
	}
	struct FScalarMaterialInput GetPixelDepthOffset() {
		return memory.read<struct FScalarMaterialInput>(m_addr + 412);
	}
	struct FShadingModelMaterialInput GetShadingModelFromMaterialExpression() {
		return memory.read<struct FShadingModelMaterialInput>(m_addr + 432);
	}
	char GetbEnableSeparateTranslucency() {
		return memory.read<char>(m_addr + 452);
	}
	...
	char ETranslucencyLightingMode GetTranslucencyLightingMode() {
		return memory.read<char ETranslucencyLightingMode>(m_addr + 453);
	}
	char GetbEnableMobileSeparateTranslucency() {
		return memory.read<char>(m_addr + 454);
	}
	...
	char GetbUsedWithEditorCompositing() {
		return memory.read<char>(m_addr + 505);
	}
	...
	char GetbUsedWithStaticLighting() {
		return memory.read<char>(m_addr + 506);
	}
	...
	char GetbUsedWithUI() {
		return memory.read<char>(m_addr + 512);
	}
	...
	char GetbForwardRenderUsePreintegratedGFForSimpleIBL() {
		return memory.read<char>(m_addr + 516);
	}
	...
	char EMaterialTessellationMode GetD3D11TessellationMode() {
		return memory.read<char EMaterialTessellationMode>(m_addr + 521);
	}
	...
	char EMaterialShadingRate GetShadingRate() {
		return memory.read<char EMaterialShadingRate>(m_addr + 523);
	}
	...
	char EBlendableLocation GetBlendableLocation() {
		return memory.read<char EBlendableLocation>(m_addr + 526);
	}
	char GetBlendableOutputAlpha() {
		return memory.read<char>(m_addr + 527);
	}
	char GetbEnableStencilTest() {
		return memory.read<char>(m_addr + 527);
	}
	char EMaterialStencilCompare GetStencilCompare() {
		return memory.read<char EMaterialStencilCompare>(m_addr + 528);
	}
	char GetStencilRefValue() {
		return memory.read<char>(m_addr + 529);
	}
	char ERefractionMode GetRefractionMode() {
		return memory.read<char ERefractionMode>(m_addr + 530);
	}
	...
	uint32_t GetUsageFlagWarnings() {
		return memory.read<uint32_t>(m_addr + 540);
	}
	float GetRefractionDepthBias() {
		return memory.read<float>(m_addr + 544);
	}
	struct FGuid GetStateId() {
		return memory.read<struct FGuid>(m_addr + 548);
	}
	float GetMaxDisplacement() {
		return memory.read<float>(m_addr + 564);
	}
	struct FMaterialCachedExpressionData GetCachedExpressionData() {
		return memory.read<struct FMaterialCachedExpressionData>(m_addr + 616);
	}
...
public:
	...
	char EAdditiveAnimationType GetAdditiveAnimType() {
		return memory.read<char EAdditiveAnimationType>(m_addr + 336);
	}
	char EAdditiveBasePoseType GetRefPoseType() {
		return memory.read<char EAdditiveBasePoseType>(m_addr + 337);
	}
	...
	uint8_t  GetInterpolation() {
		return memory.read<uint8_t >(m_addr + 384);
	}
	bool GetbEnableRootMotion() {
		return memory.read<bool>(m_addr + 385);
	}
	char ERootMotionRootLock GetRootMotionRootLock() {
		return memory.read<char ERootMotionRootLock>(m_addr + 386);
	}
...
public:
	...
	char EStreamingVolumeUsage GetStreamingUsage() {
		return memory.read<char EStreamingVolumeUsage>(m_addr + 620);
	}
...
public:
	...
	char ECollisionChannel GetSpringChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 520);
	}
	bool GetbIgnoreSelf() {
...
public:
	...
	char ERootMotionRootLock GetRootMotionRootLock() {
		return memory.read<char ERootMotionRootLock>(m_addr + 217);
	}
	bool GetbForceRootLock() {
...
public:
	...
	char TextureGroup GetLODGroup() {
		return memory.read<char TextureGroup>(m_addr + 64);
	}
	int32_t GetSize() {
		return memory.read<int32_t>(m_addr + 68);
	}
...
public:
	...
	struct FAtmospherePrecomputeParameters GetPrecomputeParams() {
		return memory.read<struct FAtmospherePrecomputeParameters>(m_addr + 556);
	}
...
public:
	...
	uint8_t  GetVoiPSampleRate() {
		return memory.read<uint8_t >(m_addr + 272);
	}
	...
	uint8_t  GetPanningMethod() {
		return memory.read<uint8_t >(m_addr + 320);
	}
	uint8_t  GetMonoChannelUpmixMethod() {
		return memory.read<uint8_t >(m_addr + 321);
	}
...
public:
	...
	char EDepthOfFieldFunctionValue GetFunctionValue() {
		return memory.read<char EDepthOfFieldFunctionValue>(m_addr + 64);
	}
...
public:
	...
	uint8_t  GetTeleportType() {
		return memory.read<uint8_t >(m_addr + 249);
	}
	uint8_t  GetBehaviourType() {
		return memory.read<uint8_t >(m_addr + 250);
	}
...
public:
	...
	uint8_t  GetReflectionSourceType() {
		return memory.read<uint8_t >(m_addr + 512);
	}
	uint8_t  GetMobileReflectionCompression() {
		return memory.read<uint8_t >(m_addr + 513);
	}
	...
	struct FGuid GetMapBuildDataId() {
		return memory.read<struct FGuid>(m_addr + 560);
	}
...
public:
	...
	uint8_t  GetAttenuation() {
		return memory.read<uint8_t >(m_addr + 504);
	}
...
public:
	...
	char TextureAddress GetAddressX() {
		return memory.read<char TextureAddress>(m_addr + 408);
	}
	char TextureAddress GetAddressY() {
		return memory.read<char TextureAddress>(m_addr + 409);
	}
	char GetbForceLinearGamma() {
	...
	char ETextureRenderTargetFormat GetRenderTargetFormat() {
		return memory.read<char ETextureRenderTargetFormat>(m_addr + 411);
	}
	char GetbAutoGenerateMips() {
		return memory.read<char>(m_addr + 412);
	}
	char TextureFilter GetMipsSamplerFilter() {
		return memory.read<char TextureFilter>(m_addr + 413);
	}
	char TextureAddress GetMipsAddressU() {
		return memory.read<char TextureAddress>(m_addr + 414);
	}
	char TextureAddress GetMipsAddressV() {
		return memory.read<char TextureAddress>(m_addr + 415);
	}
	char EPixelFormat GetOverrideFormat() {
		return memory.read<char EPixelFormat>(m_addr + 416);
	}
...
public:
	...
	char GetbRenderLightMapDensityGrayscale() {
		return memory.read<char>(m_addr + 1596);
	}
	...
	struct FLinearColor GetLightMapDensityVertexMappedColor() {
		return memory.read<struct FLinearColor>(m_addr + 1608);
	}
	struct FLinearColor GetLightMapDensitySelectedColor() {
		return memory.read<struct FLinearColor>(m_addr + 1624);
	}
	struct TArray<struct FStatColorMapping> GetStatColorMappings() {
		return memory.read<struct TArray<struct FStatColorMapping>>(m_addr + 1640);
	}
	...
	struct FSoftObjectPath GetDefaultPhysMaterialName() {
		return memory.read<struct FSoftObjectPath>(m_addr + 1664);
	}
	struct TArray<struct FGameNameRedirect> GetActiveGameNameRedirects() {
		return memory.read<struct TArray<struct FGameNameRedirect>>(m_addr + 1688);
	}
	struct TArray<struct FClassRedirect> GetActiveClassRedirects() {
		return memory.read<struct TArray<struct FClassRedirect>>(m_addr + 1704);
	}
	struct TArray<struct FPluginRedirect> GetActivePluginRedirects() {
		return memory.read<struct TArray<struct FPluginRedirect>>(m_addr + 1720);
	}
	struct TArray<struct FStructRedirect> GetActiveStructRedirects() {
		return memory.read<struct TArray<struct FStructRedirect>>(m_addr + 1736);
	}
	...
	struct FSoftObjectPath GetPreIntegratedSkinBRDFTextureName() {
		return memory.read<struct FSoftObjectPath>(m_addr + 1760);
	}
	...
	struct FSoftObjectPath GetBlueNoiseTextureName() {
		return memory.read<struct FSoftObjectPath>(m_addr + 1792);
	}
	...
	struct FSoftObjectPath GetMiniFontTextureName() {
		return memory.read<struct FSoftObjectPath>(m_addr + 1824);
	}
	...
	struct FSoftObjectPath GetWeightMapPlaceholderTextureName() {
		return memory.read<struct FSoftObjectPath>(m_addr + 1856);
	}
	...
	struct FSoftObjectPath GetLightMapDensityTextureName() {
		return memory.read<struct FSoftObjectPath>(m_addr + 1888);
	}
	...
	struct TArray<struct FString> GetDeferredCommands() {
		return memory.read<struct TArray<struct FString>>(m_addr + 1928);
	}
	float GetNearClipPlane() {
		return memory.read<float>(m_addr + 1944);
	}
	char GetbSubtitlesEnabled() {
		return memory.read<char>(m_addr + 1948);
	}
	char GetbSubtitlesForcedOff() {
		return memory.read<char>(m_addr + 1948);
	}
	int32_t GetMaximumLoopIterationCount() {
		return memory.read<int32_t>(m_addr + 1952);
	}
	char GetbCanBlueprintsTickByDefault() {
		return memory.read<char>(m_addr + 1956);
	}
	...
	float GetFixedFrameRate() {
		return memory.read<float>(m_addr + 1960);
	}
	struct FFloatRange GetSmoothedFrameRateRange() {
		return memory.read<struct FFloatRange>(m_addr + 1964);
	}
	...
	struct FSoftClassPath GetCustomTimeStepClassName() {
		return memory.read<struct FSoftClassPath>(m_addr + 2024);
	}
	...
	char GetbCheckForMultiplePawnsSpawnedInAFrame() {
		return memory.read<char>(m_addr + 2128);
	}
	int32_t GetNumPawnsAllowedToBeSpawnedInAFrame() {
		return memory.read<int32_t>(m_addr + 2132);
	}
	char GetbShouldGenerateLowQualityLightmaps() {
		return memory.read<char>(m_addr + 2136);
	}
	struct FColor GetC_WorldBox() {
		return memory.read<struct FColor>(m_addr + 2140);
	}
	struct FColor GetC_BrushWire() {
		return memory.read<struct FColor>(m_addr + 2144);
	}
	struct FColor GetC_AddWire() {
		return memory.read<struct FColor>(m_addr + 2148);
	}
	struct FColor GetC_SubtractWire() {
		return memory.read<struct FColor>(m_addr + 2152);
	}
	struct FColor GetC_SemiSolidWire() {
		return memory.read<struct FColor>(m_addr + 2156);
	}
	struct FColor GetC_NonSolidWire() {
		return memory.read<struct FColor>(m_addr + 2160);
	}
	...
	float GetStreamingDistanceFactor() {
		return memory.read<float>(m_addr + 2192);
	}
	struct FDirectoryPath GetGameScreenshotSaveDirectory() {
		return memory.read<struct FDirectoryPath>(m_addr + 2200);
	}
	...
	struct FString GetTransitionGameMode() {
		return memory.read<struct FString>(m_addr + 2240);
	}
	char GetbAllowMatureLanguage() {
		return memory.read<char>(m_addr + 2256);
	}
	...
	char GetbPauseOnLossOfFocus() {
		return memory.read<char>(m_addr + 2276);
	}
	int32_t GetMaxParticleResize() {
		return memory.read<int32_t>(m_addr + 2280);
	}
	int32_t GetMaxParticleResizeWarn() {
		return memory.read<int32_t>(m_addr + 2284);
	}
	struct TArray<struct FDropNoteInfo> GetPendingDroppedNotes() {
		return memory.read<struct TArray<struct FDropNoteInfo>>(m_addr + 2288);
	}
	...
	uint32_t GetbEnableVisualLogRecordingOnStart() {
		return memory.read<uint32_t>(m_addr + 2404);
	}
	int32_t GetScreenSaverInhibitorSemaphore() {
		return memory.read<int32_t>(m_addr + 2408);
	}
	char GetbLockReadOnlyLevels() {
		return memory.read<char>(m_addr + 2412);
	}
	struct FString GetParticleEventManagerClassPath() {
		return memory.read<struct FString>(m_addr + 2416);
	}
	...
	struct TArray<struct FNetDriverDefinition> GetNetDriverDefinitions() {
		return memory.read<struct TArray<struct FNetDriverDefinition>>(m_addr + 3064);
	}
	struct TArray<struct FString> GetServerActors() {
		return memory.read<struct TArray<struct FString>>(m_addr + 3080);
	}
	struct TArray<struct FString> GetRuntimeServerActors() {
		return memory.read<struct TArray<struct FString>>(m_addr + 3096);
	}
	float GetNetErrorLogInterval() {
		return memory.read<float>(m_addr + 3112);
	}
	char GetbStartedLoadMapMovie() {
		return memory.read<char>(m_addr + 3116);
	}
	int32_t GetNextWorldContextHandle() {
		return memory.read<int32_t>(m_addr + 3144);
	}
...
public:
	...
	char EMaterialSceneAttributeInputMode GetInputMode() {
		return memory.read<char EMaterialSceneAttributeInputMode>(m_addr + 64);
	}
	struct FExpressionInput GetInput() {
		return memory.read<struct FExpressionInput>(m_addr + 68);
	}
...
public:
	...
	char GetbShowSelectedInfo() {
		return memory.read<char>(m_addr + 1392);
	}
	...
	struct FHitResult GetSelectedHitPoint() {
		return memory.read<struct FHitResult>(m_addr + 1424);
	}
...
public:
	...
	struct FVector GetDecalSize() {
		return memory.read<struct FVector>(m_addr + 540);
	}
...
public:
	...
	struct TMap<struct FString, struct FRollbackNetStartupActorInfo> GetRollbackNetStartupActors() {
		return memory.read<struct TMap<struct FString, struct FRollbackNetStartupActorInfo>>(m_addr + 2152);
	}
	float GetCheckpointSaveMaxMSPerFrame() {
		return memory.read<float>(m_addr + 2580);
	}
	struct TArray<struct FMulticastRecordOptions> GetMulticastRecordOptions() {
		return memory.read<struct TArray<struct FMulticastRecordOptions>>(m_addr + 2616);
	}
	bool GetbIsLocalReplay() {
		return memory.read<bool>(m_addr + 2632);
	}
...
public:
	...
	char EMaterialShadingModel GetShadingModel() {
		return memory.read<char EMaterialShadingModel>(m_addr + 64);
	}
...
public:
	...
	char DistributionParamMode GetParamMode() {
		return memory.read<char DistributionParamMode>(m_addr + 88);
	}
...
public:
	...
	char EDistributionVectorLockFlags GetLockedAxes() {
		return memory.read<char EDistributionVectorLockFlags>(m_addr + 72);
	}
...
public:
	...
	char DistributionParamMode GetParamModes[3]() {
		return memory.read<char DistributionParamMode>(m_addr + 136);
	}
...
public:
	...
	char EDistributionVectorLockFlags GetLockedAxes[2]() {
		return memory.read<char EDistributionVectorLockFlags>(m_addr + 84);
	}
	char EDistributionVectorMirrorFlags GetMirrorFlags[3]() {
		return memory.read<char EDistributionVectorMirrorFlags>(m_addr + 86);
	}
	char GetbUseExtremes() {
		return memory.read<char>(m_addr + 92);
	}
...
public:
	...
	char ENodeAdvancedPins GetAdvancedPinDisplay() {
		return memory.read<char ENodeAdvancedPins>(m_addr + 88);
	}
	uint8_t  GetEnabledState() {
		return memory.read<uint8_t >(m_addr + 89);
	}
...
public:
	...
	char ECameraProjectionMode GetProjectionType() {
		return memory.read<char ECameraProjectionMode>(m_addr + 688);
	}
	float GetFOVAngle() {
		return memory.read<float>(m_addr + 692);
	}
	...
	char ESceneCaptureCompositeMode GetCompositeMode() {
		return memory.read<char ESceneCaptureCompositeMode>(m_addr + 712);
	}
	struct FPostProcessSettings GetPostProcessSettings() {
		return memory.read<struct FPostProcessSettings>(m_addr + 720);
	}
	float GetPostProcessBlendWeight() {
		return memory.read<float>(m_addr + 2096);
	}
	char GetbOverride_CustomNearClippingPlane() {
		return memory.read<char>(m_addr + 2100);
	}
	float GetCustomNearClippingPlane() {
		return memory.read<float>(m_addr + 2104);
	}
	bool GetbUseCustomProjectionMatrix() {
		return memory.read<bool>(m_addr + 2108);
	}
	struct FMatrix GetCustomProjectionMatrix() {
		return memory.read<struct FMatrix>(m_addr + 2112);
	}
	bool GetbEnableClipPlane() {
		return memory.read<bool>(m_addr + 2176);
	}
	struct FVector GetClipPlaneBase() {
		return memory.read<struct FVector>(m_addr + 2180);
	}
	struct FVector GetClipPlaneNormal() {
		return memory.read<struct FVector>(m_addr + 2192);
	}
	char GetbCameraCutThisFrame() {
		return memory.read<char>(m_addr + 2204);
	}
	char GetbConsiderUnrenderedOpaquePixelAsFullyTranslucent() {
		return memory.read<char>(m_addr + 2204);
	}
	bool GetbDisableFlipCopyGLES() {
		return memory.read<bool>(m_addr + 2208);
	}
...
public:
	...
	uint8_t  GetLoadingPolicy() {
		return memory.read<uint8_t >(m_addr + 65);
	}
	uint8_t  GetLayoutMethod() {
		return memory.read<uint8_t >(m_addr + 66);
	}
...
public:
	...
	struct FForceFeedbackAttenuationSettings GetAttenuationOverrides() {
		return memory.read<struct FForceFeedbackAttenuationSettings>(m_addr + 528);
	}
...
public:
	...
	char EParticleBurstMethod GetParticleBurstMethod() {
		return memory.read<char EParticleBurstMethod>(m_addr + 152);
	}
...
public:
	...
	char EFunctionInputType GetInputType() {
		return memory.read<char EFunctionInputType>(m_addr + 128);
	}
...
public:
	...
	char EPixelFormat GetOverrideFormat() {
		return memory.read<char EPixelFormat>(m_addr + 412);
	}
	char GetbHDR() {
...
public:
	...
	char ERadialImpulseFalloff GetFalloff() {
		return memory.read<char ERadialImpulseFalloff>(m_addr + 508);
	}
	...
	struct TArray<char EObjectTypeQuery> GetObjectTypesToAffect() {
		return memory.read<struct TArray<char EObjectTypeQuery>>(m_addr + 528);
	}
...
public:
	...
	char EAttractorParticleSelectionMethod GetSelectionMethod() {
		return memory.read<char EAttractorParticleSelectionMethod>(m_addr + 164);
	}
...
public:
	...
	char ETrackActiveCondition GetActiveCondition() {
		return memory.read<char ETrackActiveCondition>(m_addr + 80);
	}
...
public:
	...
	struct FMulticastInlineDelegate GetOnLevelUnloaded() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 248);
	}
...
public:
	...
	char ETrackToggleAction GetAction() {
		return memory.read<char ETrackToggleAction>(m_addr + 40);
	}
	float GetLastUpdatePosition() {
		return memory.read<float>(m_addr + 44);
	}
...
public:
	...
	char EVisibilityTrackAction GetAction() {
		return memory.read<char EVisibilityTrackAction>(m_addr + 40);
	}
	float GetLastUpdatePosition() {
		return memory.read<float>(m_addr + 44);
	}
...
public:
	...
	char EInterpTrackMoveRotMode GetRotMode() {
		return memory.read<char EInterpTrackMoveRotMode>(m_addr + 196);
	}
...
public:
	...
	char Beam2SourceTargetMethod GetSourceMethod() {
		return memory.read<char Beam2SourceTargetMethod>(m_addr + 48);
	}
	...
	char Beam2SourceTargetTangentMethod GetSourceTangentMethod() {
		return memory.read<char Beam2SourceTargetTangentMethod>(m_addr + 140);
	}
...
public:
	...
	char EMeshScreenAlignment GetMeshAlignment() {
		return memory.read<char EMeshScreenAlignment>(m_addr + 69);
	}
	...
	char EParticleAxisLock GetAxisLockOption() {
		return memory.read<char EParticleAxisLock>(m_addr + 144);
	}
	char GetbCameraFacing() {
		return memory.read<char>(m_addr + 145);
	}
	char EMeshCameraFacingUpAxis GetCameraFacingUpAxisOption() {
		return memory.read<char EMeshCameraFacingUpAxis>(m_addr + 146);
	}
	char EMeshCameraFacingOptions GetCameraFacingOption() {
		return memory.read<char EMeshCameraFacingOptions>(m_addr + 147);
	}
...
public:
	...
	struct TArray<struct FReplicatedStaticActorDestructionInfo> GetDestroyedReplicatedStaticActors() {
		return memory.read<struct TArray<struct FReplicatedStaticActorDestructionInfo>>(m_addr + 648);
	}
...
public:
	...
	struct FColor GetGroundAlbedo() {
		return memory.read<struct FColor>(m_addr + 524);
	}
...
public:
	...
	char ESceneTextureId GetSceneTextureId() {
		return memory.read<char ESceneTextureId>(m_addr + 84);
	}
	bool GetbFiltered() {
		return memory.read<bool>(m_addr + 85);
	}
...
public:
	...
	char ERuntimeVirtualTextureMipValueMode GetMipValueMode() {
		return memory.read<char ERuntimeVirtualTextureMipValueMode>(m_addr + 139);
	}
	char ERuntimeVirtualTextureTextureAddressMode GetTextureAddressMode() {
		return memory.read<char ERuntimeVirtualTextureTextureAddressMode>(m_addr + 140);
	}
...
public:
	...
	char ELightingBuildQuality GetLevelLightingQuality() {
		return memory.read<char ELightingBuildQuality>(m_addr + 40);
	}
...
public:
	...
	char ETextureColorChannel GetChannel() {
		return memory.read<char ETextureColorChannel>(m_addr + 132);
	}
...
public:
	...
	char EMaterialPositionTransformSource GetTransformSourceType() {
		return memory.read<char EMaterialPositionTransformSource>(m_addr + 84);
	}
	char EMaterialPositionTransformSource GetTransformType() {
		return memory.read<char EMaterialPositionTransformSource>(m_addr + 85);
	}
...
public:
	...
	char EMaterialAttributeBlend GetPixelAttributeBlendType() {
		return memory.read<char EMaterialAttributeBlend>(m_addr + 132);
	}
	char EMaterialAttributeBlend GetVertexAttributeBlendType() {
		return memory.read<char EMaterialAttributeBlend>(m_addr + 133);
	}
...
public:
	...
	char EChannelMaskParameterColor GetMaskChannel() {
		return memory.read<char EChannelMaskParameterColor>(m_addr + 112);
	}
...
public:
	...
	char EWorldPositionIncludedOffsets GetWorldPositionShaderOffset() {
		return memory.read<char EWorldPositionIncludedOffsets>(m_addr + 64);
	}
...
public:
	...
	uint8_t  GetPrimitiveRenderMode() {
		return memory.read<uint8_t >(m_addr + 504);
	}
	char ESceneCaptureSource GetCaptureSource() {
		return memory.read<char ESceneCaptureSource>(m_addr + 505);
	}
	...
	bool GetbAlwaysPersistRenderingState() {
		return memory.read<bool>(m_addr + 507);
	}
	struct TArray<struct TWeakObjectPtr<UPrimitiveComponent>> GetHiddenComponents() {
		return memory.read<struct TArray<struct TWeakObjectPtr<UPrimitiveComponent>>>(m_addr + 512);
	}
	struct TArray<struct AActor> GetHiddenActors() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 528);
		return struct TArray<struct AActor>(ptr_addr);
	...
	struct TArray<struct TWeakObjectPtr<UPrimitiveComponent>> GetShowOnlyComponents() {
		return memory.read<struct TArray<struct TWeakObjectPtr<UPrimitiveComponent>>>(m_addr + 544);
	}
	...
	struct TArray<struct FEngineShowFlagsSetting> GetShowFlagSettings() {
		return memory.read<struct TArray<struct FEngineShowFlagsSetting>>(m_addr + 592);
	}
...
public:
	...
	char ENoiseFunction GetNoiseFunction() {
		return memory.read<char ENoiseFunction>(m_addr + 112);
	}
	char GetbTurbulence() {
...
public:
	...
	struct FColor GetGroundAlbedo() {
		return memory.read<struct FColor>(m_addr + 512);
	}
	...
	struct FLinearColor GetRayleighScattering() {
		return memory.read<struct FLinearColor>(m_addr + 532);
	}
	...
	struct FTentDistribution GetOtherTentDistribution() {
		return memory.read<struct FTentDistribution>(m_addr + 620);
	}
	...
	struct FGuid GetbStaticLightingBuiltGUID() {
		return memory.read<struct FGuid>(m_addr + 700);
	}
...
public:
	...
	char EFieldVectorType GetFieldTarget() {
		return memory.read<char EFieldVectorType>(m_addr + 84);
	}
...
public:
	...
	char EFieldScalarType GetFieldTarget() {
		return memory.read<char EFieldScalarType>(m_addr + 84);
	}
...
public:
	...
	char EFieldIntegerType GetFieldTarget() {
		return memory.read<char EFieldIntegerType>(m_addr + 84);
	}
...
public:
	...
	char ESpeedTreeGeometryType GetGeometryType() {
		return memory.read<char ESpeedTreeGeometryType>(m_addr + 144);
	}
	char ESpeedTreeWindType GetWindType() {
		return memory.read<char ESpeedTreeWindType>(m_addr + 145);
	}
	char ESpeedTreeLODType GetLODType() {
		return memory.read<char ESpeedTreeLODType>(m_addr + 146);
	}
...
public:
	...
	char EMaterialExposedTextureProperty GetProperty() {
		return memory.read<char EMaterialExposedTextureProperty>(m_addr + 84);
	}
...
public:
	...
	char EMaterialVectorCoordTransformSource GetTransformSourceType() {
		return memory.read<char EMaterialVectorCoordTransformSource>(m_addr + 84);
	}
	char EMaterialVectorCoordTransform GetTransformType() {
		return memory.read<char EMaterialVectorCoordTransform>(m_addr + 85);
	}
...
public:
	...
	char EVectorNoiseFunction GetNoiseFunction() {
		return memory.read<char EVectorNoiseFunction>(m_addr + 84);
	}
	int32_t GetQuality() {
		return memory.read<int32_t>(m_addr + 88);
	}
...
public:
	...
	struct FConstrainComponentPropName GetComponentName1() {
		return memory.read<struct FConstrainComponentPropName>(m_addr + 512);
	}
	struct AActor GetConstraintActor2() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 520);
		return struct AActor(ptr_addr);
	...
	struct FConstraintInstance GetConstraintInstance() {
		return memory.read<struct FConstraintInstance>(m_addr + 576);
	}
...
public:
	...
	char EEmitterRenderMode GetEmitterRenderMode() {
		return memory.read<char EEmitterRenderMode>(m_addr + 52);
	}
	uint8_t  GetSignificanceLevel() {
		return memory.read<uint8_t >(m_addr + 53);
	}
...
public:
	...
	char Beam2SourceTargetMethod GetTargetMethod() {
		return memory.read<char Beam2SourceTargetMethod>(m_addr + 48);
	}
	...
	char Beam2SourceTargetTangentMethod GetTargetTangentMethod() {
		return memory.read<char Beam2SourceTargetTangentMethod>(m_addr + 140);
	}
...
public:
	...
	char EParticleCameraOffsetUpdateMethod GetUpdateMethod() {
		return memory.read<char EParticleCameraOffsetUpdateMethod>(m_addr + 100);
	}
...
public:
	...
	char EParticleCollisionComplete GetCollisionCompletionOption() {
		return memory.read<char EParticleCollisionComplete>(m_addr + 240);
	}
	struct TArray<char EObjectTypeQuery> GetCollisionTypes() {
		return memory.read<struct TArray<char EObjectTypeQuery>>(m_addr + 248);
	}
...
public:
	...
	char EParticleEventType GetEventGeneratorType() {
		return memory.read<char EParticleEventType>(m_addr + 48);
	}
...
public:
	...
	char ELocationBoneSocketSource GetSourceType() {
		return memory.read<char ELocationBoneSocketSource>(m_addr + 48);
	}
	...
	char ELocationBoneSocketSelectionMethod GetSelectionMethod() {
		return memory.read<char ELocationBoneSocketSelectionMethod>(m_addr + 80);
	}
...
public:
	...
	char ELocationEmitterSelectionMethod GetSelectionMethod() {
		return memory.read<char ELocationEmitterSelectionMethod>(m_addr + 56);
	}
...
public:
	...
	char ELocationSkelVertSurfaceSource GetSourceType() {
		return memory.read<char ELocationSkelVertSurfaceSource>(m_addr + 48);
	}
	struct FVector GetUniversalOffset() {
		return memory.read<struct FVector>(m_addr + 52);
	}
...
public:
	...
	char EParticleScreenAlignment GetScreenAlignment() {
		return memory.read<char EParticleScreenAlignment>(m_addr + 88);
	}
	...
	char EParticleSortMode GetSortMode() {
		return memory.read<char EParticleSortMode>(m_addr + 90);
	}
	...
	char EParticleSubUVInterpMethod GetInterpolationMethod() {
		return memory.read<char EParticleSubUVInterpMethod>(m_addr + 169);
	}
	char GetbScaleUV() {
	...
	char EParticleBurstMethod GetParticleBurstMethod() {
		return memory.read<char EParticleBurstMethod>(m_addr + 171);
	}
	...
	char EOpacitySourceMode GetOpacitySourceMode() {
		return memory.read<char EOpacitySourceMode>(m_addr + 173);
	}
	char EEmitterNormalsMode GetEmitterNormalsMode() {
		return memory.read<char EEmitterNormalsMode>(m_addr + 174);
	}
	...
	uint8_t  GetUVFlippingMode() {
		return memory.read<uint8_t >(m_addr + 208);
	}
	char ESubUVBoundingVertexCount GetBoundingMode() {
		return memory.read<char ESubUVBoundingVertexCount>(m_addr + 209);
	}
...
public:
	...
	struct FPostProcessSettings GetSettings() {
		return memory.read<struct FPostProcessSettings>(m_addr + 512);
	}
	float GetPriority() {
	...
	char GetbEnabled() {
		return memory.read<char>(m_addr + 1900);
	}
	char GetbUnbound() {
		return memory.read<char>(m_addr + 1900);
	}
...
public:
	...
	char GetbResetOnDetach() {
		return memory.read<char>(m_addr + 1145);
	}
	...
	char GetbWarmingUp() {
		return memory.read<char>(m_addr + 1146);
	}
	...
	char ParticleSystemLODMethod GetLODMethod() {
		return memory.read<char ParticleSystemLODMethod>(m_addr + 1157);
	}
	uint8_t  GetRequiredSignificance() {
		return memory.read<uint8_t >(m_addr + 1158);
	}
	struct TArray<struct FParticleSysParam> GetInstanceParameters() {
		return memory.read<struct TArray<struct FParticleSysParam>>(m_addr + 1160);
	}
	struct FMulticastInlineDelegate GetOnParticleSpawn() {
	...
	struct FVector GetPartSysVelocity() {
		return memory.read<struct FVector>(m_addr + 1256);
	}
	float GetWarmupTime() {
	...
	struct TWeakObjectPtr<USceneComponent> GetAutoAttachParent() {
		return memory.read<struct TWeakObjectPtr<USceneComponent>>(m_addr + 1432);
	}
	struct FName GetAutoAttachSocketName() {
		return memory.read<struct FName>(m_addr + 1440);
	}
	uint8_t  GetAutoAttachLocationRule() {
		return memory.read<uint8_t >(m_addr + 1448);
	}
	uint8_t  GetAutoAttachRotationRule() {
		return memory.read<uint8_t >(m_addr + 1449);
	}
	uint8_t  GetAutoAttachScaleRule() {
		return memory.read<uint8_t >(m_addr + 1450);
	}
	struct FMulticastInlineDelegate GetOnSystemFinished() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1496);
	}
...
public:
	...
	char ETrail2SourceMethod GetSourceMethod() {
		return memory.read<char ETrail2SourceMethod>(m_addr + 48);
	}
	struct FName GetSourceName() {
		return memory.read<struct FName>(m_addr + 52);
	}
	...
	char EParticleSourceSelectionMethod GetSelectionMethod() {
		return memory.read<char EParticleSourceSelectionMethod>(m_addr + 136);
	}
...
public:
	...
	char TextureAddress GetAddressX() {
		return memory.read<char TextureAddress>(m_addr + 44);
	}
	char TextureAddress GetAddressY() {
		return memory.read<char TextureAddress>(m_addr + 45);
	}
...
public:
	...
	char ESettingsLockedAxis GetLockedAxis() {
		return memory.read<char ESettingsLockedAxis>(m_addr + 276);
	}
	char ESettingsDOF GetDefaultDegreesOfFreedom() {
		return memory.read<char ESettingsDOF>(m_addr + 277);
	}
	...
	struct FChaosPhysicsSettings GetChaosSettings() {
		return memory.read<struct FChaosPhysicsSettings>(m_addr + 384);
	}
...
public:
	...
	char EMobileMSAASampleCount GetMobileMSAASampleCount() {
		return memory.read<char EMobileMSAASampleCount>(m_addr + 64);
	}
	...
	char ETranslucentSortPolicy GetTranslucentSortPolicy() {
		return memory.read<char ETranslucentSortPolicy>(m_addr + 120);
	}
	struct FVector GetTranslucentSortAxis() {
		return memory.read<struct FVector>(m_addr + 124);
	}
	char EFixedFoveationLevels GetHMDFixedFoveationLevel() {
		return memory.read<char EFixedFoveationLevels>(m_addr + 136);
	}
	char ECustomDepthStencil GetCustomDepthStencil() {
		return memory.read<char ECustomDepthStencil>(m_addr + 137);
	}
	char GetbCustomDepthTaaJitter() {
		return memory.read<char>(m_addr + 140);
	}
	char EAlphaChannelMode GetbEnableAlphaChannelInPostProcessing() {
		return memory.read<char EAlphaChannelMode>(m_addr + 144);
	}
	...
	char EAutoExposureMethodUI GetDefaultFeatureAutoExposure() {
		return memory.read<char EAutoExposureMethodUI>(m_addr + 152);
	}
	...
	char EAntiAliasingMethod GetDefaultFeatureAntiAliasing() {
		return memory.read<char EAntiAliasingMethod>(m_addr + 164);
	}
	uint8_t  GetDefaultLightUnits() {
		return memory.read<uint8_t >(m_addr + 165);
	}
	char EDefaultBackBufferPixelFormat GetDefaultBackBufferPixelFormat() {
		return memory.read<char EDefaultBackBufferPixelFormat>(m_addr + 166);
	}
	...
	char EEarlyZPass GetEarlyZPass() {
		return memory.read<char EEarlyZPass>(m_addr + 172);
	}
	...
	char EClearSceneOptions GetClearSceneMethod() {
		return memory.read<char EClearSceneOptions>(m_addr + 180);
	}
	...
	char EGBufferFormat GetGBufferFormat() {
		return memory.read<char EGBufferFormat>(m_addr + 200);
	}
	...
	uint8_t  GetDefaultSkinCacheBehavior() {
	...
	char EMobilePlanarReflectionMode GetMobilePlanarReflectionMode() {
		return memory.read<char EMobilePlanarReflectionMode>(m_addr + 244);
	}
...
public:
	...
	struct TSoftObjectPtr<AActor> GetBoundsAlignActor() {
		return memory.read<struct TSoftObjectPtr<AActor>>(m_addr + 504);
	}
...
public:
	...
	char EShadowMapFlags GetShadowmapFlags() {
		return memory.read<char EShadowMapFlags>(m_addr + 416);
	}
...
public:
	...
	char EAxis GetSkelMirrorAxis() {
		return memory.read<char EAxis>(m_addr + 349);
	}
	char EAxis GetSkelMirrorFlipAxis() {
		return memory.read<char EAxis>(m_addr + 350);
	}
...
public:
	...
	char ESkyLightSourceType GetSourceType() {
		return memory.read<char ESkyLightSourceType>(m_addr + 553);
	}
	struct UTextureCube GetCubemap() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 560);
		return struct UTextureCube(ptr_addr);
	...
	struct FLinearColor GetUpperHemisphereColor() {
	...
	char EOcclusionCombineMode GetOcclusionCombineMode() {
		return memory.read<char EOcclusionCombineMode>(m_addr + 656);
	}
...
public:
	...
	char ESubUVBoundingVertexCount GetBoundingMode() {
		return memory.read<char ESubUVBoundingVertexCount>(m_addr + 56);
	}
	char EOpacitySourceMode GetOpacitySourceMode() {
		return memory.read<char EOpacitySourceMode>(m_addr + 57);
	}
	float GetAlphaThreshold() {
		return memory.read<float>(m_addr + 60);
	}
...
public:
	...
	char TextureAddress GetAddressX() {
		return memory.read<char TextureAddress>(m_addr + 464);
	}
	char TextureAddress GetAddressY() {
		return memory.read<char TextureAddress>(m_addr + 465);
	}
	char TextureAddress GetAddressZ() {
		return memory.read<char TextureAddress>(m_addr + 466);
	}
...
public:
	...
	char EPixelFormat GetFormat() {
		return memory.read<char EPixelFormat>(m_addr + 384);
	}
...
public:
	...
	uint8_t  GetRenderFocusRule() {
		return memory.read<uint8_t >(m_addr + 56);
	}
	struct TMap<char EMouseCursor, struct FHardwareCursorReference> GetHardwareCursors() {
		return memory.read<struct TMap<char EMouseCursor, struct FHardwareCursorReference>>(m_addr + 64);
	}
	struct TMap<char EMouseCursor, struct FSoftClassPath> GetSoftwareCursors() {
		return memory.read<struct TMap<char EMouseCursor, struct FSoftClassPath>>(m_addr + 144);
	}
	...
	uint8_t  GetUIScaleRule() {
		return memory.read<uint8_t >(m_addr + 396);
	}
...
public:
	...
	char EVectorFieldConstructionOp GetConstructionOp() {
		return memory.read<char EVectorFieldConstructionOp>(m_addr + 80);
	}
