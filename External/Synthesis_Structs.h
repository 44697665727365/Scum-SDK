...
public:
	...
	uint8_t  GetLFO1Mode() {
		return memory.read<uint8_t >(m_addr + 85);
	}
	uint8_t  GetLFO1PatchType() {
		return memory.read<uint8_t >(m_addr + 86);
	}
	...
	uint8_t  GetModEnvPatchType() {
		return memory.read<uint8_t >(m_addr + 120);
	}
	uint8_t  GetModEnvBiasPatchType() {
	...
	uint8_t  GetFilterType() {
		return memory.read<uint8_t >(m_addr + 160);
	}
	uint8_t  GetFilterAlgorithm() {
		return memory.read<uint8_t >(m_addr + 161);
	}
	char GetbStereoDelayEnabled() {
		return memory.read<char>(m_addr + 164);
	}
	uint8_t  GetStereoDelayMode() {
...
public:
	...
	uint8_t  GetDynamicsProcessorType() {
		return memory.read<uint8_t >(m_addr + 0);
	}
	uint8_t  GetPeakMode() {
...
public:
	...
	uint8_t  GetFilterType() {
		return memory.read<uint8_t >(m_addr + 25);
	}
	float GetFilterFrequency() {
		return memory.read<float>(m_addr + 28);
	}
