...
public:
	...
	uint8_t  GetFrontAxisEnum() {
		return memory.read<uint8_t >(m_addr + 432);
	}
	uint8_t  GetUpAxisEnum() {
		return memory.read<uint8_t >(m_addr + 433);
	}
	char GetbFollow() {
...
public:
	...
	uint8_t  GetAttachmentLocationRule() {
		return memory.read<uint8_t >(m_addr + 296);
	}
	uint8_t  GetAttachmentRotationRule() {
		return memory.read<uint8_t >(m_addr + 297);
	}
	uint8_t  GetAttachmentScaleRule() {
		return memory.read<uint8_t >(m_addr + 298);
	}
	uint8_t  GetDetachmentLocationRule() {
		return memory.read<uint8_t >(m_addr + 299);
	}
	uint8_t  GetDetachmentRotationRule() {
		return memory.read<uint8_t >(m_addr + 300);
	}
	uint8_t  GetDetachmentScaleRule() {
...
public:
	...
	uint8_t  GetEventPosition() {
		return memory.read<uint8_t >(m_addr + 164);
	}
	struct TArray<struct UMovieSceneSection> GetSections() {
