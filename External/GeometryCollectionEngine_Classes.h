...
public:
	...
	uint8_t  GetClusterConnectionType() {
	...
	uint8_t  GetImplicitType() {
		return memory.read<uint8_t >(m_addr + 121);
	}
...
public:
	...
	struct FChaosCollisionEventRequestSettings GetCollisionEventRequestSettings() {
		return memory.read<struct FChaosCollisionEventRequestSettings>(m_addr + 508);
	}
	struct FChaosBreakingEventRequestSettings GetBreakingEventRequestSettings() {
		return memory.read<struct FChaosBreakingEventRequestSettings>(m_addr + 532);
	}
	struct FChaosTrailingEventRequestSettings GetTrailingEventRequestSettings() {
		return memory.read<struct FChaosTrailingEventRequestSettings>(m_addr + 556);
	}
...
public:
	...
	uint8_t  GetObjectType() {
		return memory.read<uint8_t >(m_addr + 186);
	}
	float GetMass() {
	...
	uint8_t  GetImplicitType() {
		return memory.read<uint8_t >(m_addr + 193);
	}
	int32_t GetMinLevelSetResolution() {
	...
	uint8_t  GetInitialVelocityType() {
		return memory.read<uint8_t >(m_addr + 204);
	}
...
public:
	...
	uint8_t  GetHideGeometry() {
		return memory.read<uint8_t >(m_addr + 579);
	}
...
public:
	...
	struct TArray<float> GetDamageThreshold() {
		return memory.read<struct TArray<float>>(m_addr + 1432);
	}
	uint8_t  GetClusterConnectionType() {
	...
	uint8_t  GetInitialVelocityType() {
		return memory.read<uint8_t >(m_addr + 1480);
	}
	struct FVector GetInitialLinearVelocity() {
		return memory.read<struct FVector>(m_addr + 1484);
	}
	struct FVector GetInitialAngularVelocity() {
		return memory.read<struct FVector>(m_addr + 1496);
	}
	...
	struct FGeomComponentCacheParameters GetCacheParameters() {
		return memory.read<struct FGeomComponentCacheParameters>(m_addr + 1520);
	}
	struct FMulticastInlineDelegate GetNotifyGeometryCollectionPhysicsStateChange() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1600);
	}
	struct FMulticastInlineDelegate GetNotifyGeometryCollectionPhysicsLoadingStateChange() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1616);
	}
	struct FMulticastInlineDelegate GetOnChaosBreakEvent() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1656);
	}
	float GetDesiredCacheTime() {
		return memory.read<float>(m_addr + 1672);
	}
	bool GetCachePlayback() {
		return memory.read<bool>(m_addr + 1676);
	}
	struct FMulticastInlineDelegate GetOnChaosPhysicsCollision() {
		return memory.read<struct FMulticastInlineDelegate>(m_addr + 1680);
	}
	...
	bool GetbEnableAbandonAfterLevel() {
		return memory.read<bool>(m_addr + 1699);
	}
	...
	struct FGeometryCollectionRepData GetRepData() {
		return memory.read<struct FGeometryCollectionRepData>(m_addr + 1704);
	}
...
public:
	...
	uint8_t  GetObjectType() {
		return memory.read<uint8_t >(m_addr + 210);
	}
	float GetDensity() {
