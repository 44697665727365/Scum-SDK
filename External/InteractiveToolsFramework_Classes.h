...
public:
	...
	struct FVector GetOrigin() {
		return memory.read<struct FVector>(m_addr + 1136);
	}
	struct FQuat GetRotation() {
		return memory.read<struct FQuat>(m_addr + 1152);
	}
...
public:
	...
	char GetSegmentFlags() {
		return memory.read<char>(m_addr + 1180);
	}
...
public:
	...
	struct FVector GetDirection() {
		return memory.read<struct FVector>(m_addr + 1156);
	}
	float GetLength() {
		return memory.read<float>(m_addr + 1168);
	}
...
public:
	...
	uint8_t  GetCurrentCoordinateSystem() {
