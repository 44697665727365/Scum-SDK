...
public:
	...
	uint8_t  GetPolygroupMode() {
		return memory.read<uint8_t >(m_addr + 97);
	}
	uint8_t  GetPlaceMode() {
		return memory.read<uint8_t >(m_addr + 98);
	}
	bool GetbSnapToGrid() {
		return memory.read<bool>(m_addr + 99);
	}
	uint8_t  GetPivotLocation() {
		return memory.read<uint8_t >(m_addr + 100);
	}
	float GetRotation() {
...
public:
	...
	uint8_t  GetDeformationStrategy() {
		return memory.read<uint8_t >(m_addr + 96);
	}
	...
	uint8_t  GetSelectedWeightScheme() {
...
public:
	...
	uint8_t  GetAlignType() {
		return memory.read<uint8_t >(m_addr + 96);
	}
	uint8_t  GetAlignTo() {
		return memory.read<uint8_t >(m_addr + 100);
	}
	uint8_t  GetBoxPosition() {
		return memory.read<uint8_t >(m_addr + 104);
	}
	bool GetbAlignX() {
		return memory.read<bool>(m_addr + 108);
...
public:
	...
	uint8_t  GetWidthMode() {
		return memory.read<uint8_t >(m_addr + 100);
	}
	float GetWidth() {
		return memory.read<float>(m_addr + 104);
	}
	uint8_t  GetHeightMode() {
		return memory.read<uint8_t >(m_addr + 108);
	}
	float GetHeight() {
		return memory.read<float>(m_addr + 112);
...
public:
	...
	uint8_t  GetMeshBoundaryConstraint() {
		return memory.read<uint8_t >(m_addr + 97);
	}
	uint8_t  GetGroupBoundaryConstraint() {
		return memory.read<uint8_t >(m_addr + 98);
	}
	uint8_t  GetMaterialBoundaryConstraint() {
		return memory.read<uint8_t >(m_addr + 99);
	}
...
public:
	...
	uint8_t  GetPositionMode() {
		return memory.read<uint8_t >(m_addr + 96);
	}
	uint8_t  GetInsertionMode() {
		return memory.read<uint8_t >(m_addr + 100);
	}
	int32_t GetNumLoops() {
		return memory.read<int32_t>(m_addr + 104);
...
public:
	...
	uint8_t  GetNormalCalculationMethod() {
		return memory.read<uint8_t >(m_addr + 97);
	}
...
public:
	...
	uint8_t  GetSelectedOperationType() {
...
public:
	...
	uint8_t  GetCtrlClickBehavior() {
		return memory.read<uint8_t >(m_addr + 101);
	}
...
public:
	...
	uint8_t  GetOcclusionTestMethod() {
		return memory.read<uint8_t >(m_addr + 96);
	}
	uint8_t  GetTriangleSampling() {
		return memory.read<uint8_t >(m_addr + 97);
	}
