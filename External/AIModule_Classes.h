...
public:
	...
	char EPawnActionFailHandling GetChildFailureHandlingMode() {
		return memory.read<char EPawnActionFailHandling>(m_addr + 160);
	}
...
public:
	...
	bool GetbHearingOcclusionEnabled() {
		return memory.read<bool>(m_addr + 96);
	}
	float GetHearingRangeReductionIfOccluded() {
		return memory.read<float>(m_addr + 100);
	}
	char ECollisionChannel GetHearingOcclusionCollisionChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 104);
	}
...
public:
	...
	char EBlackBoardEntryComparison GetOperator() {
		return memory.read<char EBlackBoardEntryComparison>(m_addr + 104);
	}
...
public:
	...
	char ECollisionChannel GetDefaultSightCollisionChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 156);
	}
...
public:
	...
	char EBTParallelMode GetFinishMode() {
		return memory.read<char EBTParallelMode>(m_addr + 144);
	}
...
public:
	...
	char EBTFlowAbortMode GetFlowAbortMode() {
		return memory.read<char EBTFlowAbortMode>(m_addr + 100);
	}
...
public:
	...
	char EBTBlackboardRestart GetNotifyObserver() {
		return memory.read<char EBTBlackboardRestart>(m_addr + 185);
	}
...
public:
	...
	char EPathExistanceQueryType GetPathQueryType() {
		return memory.read<char EPathExistanceQueryType>(m_addr + 188);
	}
...
public:
	...
	uint8_t  GetGeometricDistanceType() {
...
public:
	...
	char EBTNodeResult GetResult() {
		return memory.read<char EBTNodeResult>(m_addr + 112);
	}
...
public:
	...
	char EEnvQueryRunMode GetRunMode() {
		return memory.read<char EEnvQueryRunMode>(m_addr + 192);
	}
...
public:
	...
	uint8_t  GetPointOnCircleSpacingMethod() {
		return memory.read<uint8_t >(m_addr + 296);
	}
	struct FEnvDirection GetArcDirection() {
...
public:
	...
	char EEnvTestPurpose GetTestPurpose() {
		return memory.read<char EEnvTestPurpose>(m_addr + 52);
	}
	struct FString GetTestComment() {
		return memory.read<struct FString>(m_addr + 56);
	}
	char EEnvTestFilterOperator GetMultipleContextFilterOp() {
		return memory.read<char EEnvTestFilterOperator>(m_addr + 72);
	}
	char EEnvTestScoreOperator GetMultipleContextScoreOp() {
		return memory.read<char EEnvTestScoreOperator>(m_addr + 73);
	}
	char EEnvTestFilterType GetFilterType() {
		return memory.read<char EEnvTestFilterType>(m_addr + 74);
	}
	...
	char EEnvTestScoreEquation GetScoringEquation() {
		return memory.read<char EEnvTestScoreEquation>(m_addr + 249);
	}
	char EEnvQueryTestClamping GetClampMinType() {
		return memory.read<char EEnvQueryTestClamping>(m_addr + 250);
	}
	char EEnvQueryTestClamping GetClampMaxType() {
		return memory.read<char EEnvQueryTestClamping>(m_addr + 251);
	}
	uint8_t  GetNormalizationType() {
		return memory.read<uint8_t >(m_addr + 252);
	}
...
public:
	...
	char EEnvTestDistance GetTestMode() {
		return memory.read<char EEnvTestDistance>(m_addr + 504);
	}
...
public:
	...
	char EEnvTestPathfinding GetTestMode() {
		return memory.read<char EEnvTestPathfinding>(m_addr + 504);
	}
	UEnvQueryContext GetContext() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 512);
		return UEnvQueryContext(ptr_addr);
...
public:
	...
	struct TArray<struct FEnvNamedValue> GetQueryParams() {
		return memory.read<struct TArray<struct FEnvNamedValue>>(m_addr + 1240);
	}
	struct TArray<struct FAIDynamicParam> GetQueryConfig() {
		return memory.read<struct TArray<struct FAIDynamicParam>>(m_addr + 1256);
	}
	float GetTimeLimitPerStep() {
		return memory.read<float>(m_addr + 1272);
	}
	int32_t GetStepToDebugDraw() {
		return memory.read<int32_t>(m_addr + 1276);
	}
	uint8_t  GetHighlightMode() {
		return memory.read<uint8_t >(m_addr + 1280);
	}
	char GetbDrawLabels() {
		return memory.read<char>(m_addr + 1284);
	}
	...
	char EEnvQueryRunMode GetQueryingMode() {
		return memory.read<char EEnvQueryRunMode>(m_addr + 1288);
	}
	struct FNavAgentProperties GetNavAgentProperties() {
		return memory.read<struct FNavAgentProperties>(m_addr + 1296);
	}
