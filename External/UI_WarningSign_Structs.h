#pragma once 
#include <SDK_Engine_Objects.h> 
 
 
class FExecuteUbergraph_UI_WarningSign
{
public:
	FExecuteUbergraph_UI_WarningSign(std::uint64_t p_addr = 0) : m_addr(p_addr) {}

	std::uint64_t GetAddress() { return m_addr; }

	bool IsValidClass() { return m_addr > 0; }

	int32_t GetEntryPoint() {
		return memory.read<int32_t>(m_addr + 0);
	}
	uint8_t  GetK2Node_Event_warningType() {
		return memory.read<uint8_t >(m_addr + 4);
	}
	bool GetCallFunc_LessEqual_FloatFloat_ReturnValue() {
		return memory.read<bool>(m_addr + 5);
	}
	bool GetK2Node_SwitchEnum_CmpSuccess() {
		return memory.read<bool>(m_addr + 6);
	}
	struct UMaterialInstanceDynamic GetCallFunc_GetEffectMaterial_ReturnValue() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 8);
		return struct UMaterialInstanceDynamic(ptr_addr);
	}
	struct FGeometry GetK2Node_Event_MyGeometry() {
		return memory.read<struct FGeometry>(m_addr + 16);
	}
	float GetK2Node_Event_InDeltaTime() {
		return memory.read<float>(m_addr + 72);
	}
	struct UMaterialInstanceDynamic GetCallFunc_GetEffectMaterial_ReturnValue_2() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 80);
		return struct UMaterialInstanceDynamic(ptr_addr);
	}
	float GetCallFunc_Multiply_FloatFloat_ReturnValue() {
		return memory.read<float>(m_addr + 88);
	}
	float GetCallFunc_Subtract_FloatFloat_ReturnValue() {
		return memory.read<float>(m_addr + 92);
	}

private:
	std::uint64_t m_addr = 0;
};class FTick
{
public:
	FTick(std::uint64_t p_addr = 0) : m_addr(p_addr) {}

	std::uint64_t GetAddress() { return m_addr; }

	bool IsValidClass() { return m_addr > 0; }

	struct FGeometry GetMyGeometry() {
		return memory.read<struct FGeometry>(m_addr + 0);
	}
	float GetInDeltaTime() {
		return memory.read<float>(m_addr + 56);
	}

private:
	std::uint64_t m_addr = 0;
};class FInitFromWarningType
{
public:
	FInitFromWarningType(std::uint64_t p_addr = 0) : m_addr(p_addr) {}

	std::uint64_t GetAddress() { return m_addr; }

	bool IsValidClass() { return m_addr > 0; }

	uint8_t  GetwarningType() {
		return memory.read<uint8_t >(m_addr + 0);
	}

private:
	std::uint64_t m_addr = 0;
};