...
public:
	...
	uint8_t  GetDefaultCompletionMode() {
...
public:
	...
	char EMovieScenePlayerStatus GetStatus() {
		return memory.read<char EMovieScenePlayerStatus>(m_addr + 688);
	}
...
public:
	...
	uint8_t  GetEvaluationType() {
		return memory.read<uint8_t >(m_addr + 280);
	}
	uint8_t  GetClockSource() {
		return memory.read<uint8_t >(m_addr + 281);
	}
...
public:
	...
	uint8_t  GetAccumulatedFlags() {
		return memory.read<uint8_t >(m_addr + 1010);
	}
