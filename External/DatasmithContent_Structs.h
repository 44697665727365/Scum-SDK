...
public:
	...
	char EAutoExposureMethod GetAutoExposureMethod() {
		return memory.read<char EAutoExposureMethod>(m_addr + 48);
	}
	float GetCameraISO() {
...
public:
	...
	uint8_t  GetMinLightmapResolution() {
		return memory.read<uint8_t >(m_addr + 0);
	}
	uint8_t  GetMaxLightmapResolution() {
