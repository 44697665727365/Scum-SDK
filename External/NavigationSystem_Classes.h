...
public:
	...
	uint8_t  GetRuntimeGeneration() {
		return memory.read<uint8_t >(m_addr + 684);
	}
...
public:
	...
	char ENavLinkDirection GetLinkDirection() {
		return memory.read<char ENavLinkDirection>(m_addr + 284);
	}
	...
	char ECollisionChannel GetBroadcastChannel() {
		return memory.read<char ECollisionChannel>(m_addr + 336);
	}
...
public:
	...
	char ENavigationOptionFlag GetRecalculateOnInvalidation() {
		return memory.read<char ENavigationOptionFlag>(m_addr + 72);
	}
...
public:
	...
	char ERecastPartitioning GetRegionPartitioning() {
		return memory.read<char ERecastPartitioning>(m_addr + 1164);
	}
	char ERecastPartitioning GetLayerPartitioning() {
		return memory.read<char ERecastPartitioning>(m_addr + 1165);
	}
...
public:
	...
	uint8_t  GetOperationMode() {
		return memory.read<uint8_t >(m_addr + 508);
	}
...
public:
	...
	struct FNavGraphNode GetNode() {
		return memory.read<struct FNavGraphNode>(m_addr + 504);
	}
...
public:
	...
	ANavigationData GetNavigationDataToUpdate() {
		auto ptr_addr = memory.read<std::uint64_t>(m_addr + 184);
		return ANavigationData(ptr_addr);
	}
...
public:
	...
	char ENavCostDisplay GetCostDisplayMode() {
		return memory.read<char ENavCostDisplay>(m_addr + 684);
	}
...
public:
	...
	struct TArray<struct FNavigationLink> GetLinks() {
		return memory.read<struct TArray<struct FNavigationLink>>(m_addr + 1112);
	}
