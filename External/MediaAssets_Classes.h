...
public:
	...
	uint8_t  GetChannels() {
		return memory.read<uint8_t >(m_addr + 1728);
	}
	bool GetDynamicRateAdjustment() {
		return memory.read<bool>(m_addr + 1732);
	}
	float GetRateAdjustmentFactor() {
		return memory.read<float>(m_addr + 1736);
	}
	struct FFloatRange GetRateAdjustmentRange() {
		return memory.read<struct FFloatRange>(m_addr + 1740);
	}
	struct UMediaPlayer GetMediaPlayer() {
...
public:
	...
	char TextureAddress GetAddressX() {
		return memory.read<char TextureAddress>(m_addr + 376);
	}
	char TextureAddress GetAddressY() {
		return memory.read<char TextureAddress>(m_addr + 377);
	}
	bool GetAutoClear() {
	...
	char MediaTextureOutputFormat GetOutputFormat() {
		return memory.read<char MediaTextureOutputFormat>(m_addr + 399);
	}
	float GetCurrentAspectRatio() {
		return memory.read<float>(m_addr + 400);
	}
	char MediaTextureOrientation GetCurrentOrientation() {
		return memory.read<char MediaTextureOrientation>(m_addr + 404);
	}
	struct UMediaPlayer GetMediaPlayer() {
